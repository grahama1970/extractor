...
def _find_latest() -> Path | None:
    d1 = list(ROOT.glob("data/results/pipeline/**/14_report_generator/json_output/*.json"))
    d2 = list(ROOT.glob("out_fast/**/14_report_generator/json_output/*.json"))
    c = d1 + d2
    # Prefer canonical filename if present
    prefer = [p for p in c if p.name == "14_report.json"]
    if prefer:
        return max(prefer, key=lambda p: p.stat().st_mtime)
...
def _prompt(cand, gold):
    return "Return ONLY JSON with the rubric fields.\n" f"GOLD:\n{gold}\n\nCANDIDATE:\n{cand}\n"
...
def main():
    cand_path = os.getenv("STEP14_PATH") or (_find_latest() and str(_find_latest()))
    if not cand_path:
        print("SKIP: no STEP14_PATH and no discoverable candidate")
        sys.exit(0)
    gold_path = os.getenv("GOLD_PATH") or str(
        Path(os.getenv("GOLD_DIR", "data/gold")) / "step14_report.json"
    )
    gp = Path(gold_path)
    if not gp.exists():
        print("SKIP: gold not found for step14")
        sys.exit(0)
    try:
        cand = _load_json(Path(cand_path))
        gold = _load_json(gp)
    except Exception as e:
        print("SKIP: step14 load unavailable", e)
        sys.exit(0)
    ...
    def getf(k):
        return float(res.get(k, 0) or 0)
    passed = (
        getf("overall") >= t_overall
        and getf("structure_parity") >= t_struct
        and getf("entity_coverage") >= t_cover
        and getf("semantic_alignment") >= t_sem
        and (not res.get("critical_issues"))
    )
    out = {
    ...
    (OUT_JSON / "step_eval_step14.json").write_text(json.dumps(out, indent=2))
    print(f"Scenario pipeline/step14_eval_agent: {'pass' if passed else 'fail'}")
