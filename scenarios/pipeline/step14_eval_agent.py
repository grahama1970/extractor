...
def _find_latest() -> Path | None:
    ...
    if prefer: return max(prefer, key=lambda p: p.stat().st_mtime)
...
def _call(model: str, prompt: str):
    try:
        ...
        out = litellm_call_sync(name='step14_eval', prompts=[prompt], models=[model], response_json=True, temperature=0, extra_kwargs={"max_tokens":512})
...
def main():
    ...
    if not cand_path: print('SKIP: no STEP14_PATH and no discoverable candidate'); sys.exit(0)
    gold_path = os.getenv('GOLD_PATH') or str(Path(os.getenv('GOLD_DIR','data/gold'))/ 'step14_report.json')
    gp = Path(gold_path)
