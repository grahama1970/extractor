...
def _find_latest() -> Path | None:
    c = list(ROOT.glob("data/results/pipeline/**/07_reflow_section/json_output/07_reflowed.json"))
    c += list(ROOT.glob("out_fast/**/07_reflow_section/json_output/07_reflowed.json"))
...
def main():
    cand_path = os.getenv("STEP07_PATH") or (_find_latest() and str(_find_latest()))
    if not cand_path:
        print("SKIP: no STEP07_PATH and no discoverable candidate")
        sys.exit(0)
    gold_path = os.getenv("GOLD_PATH") or str(
        Path(os.getenv("GOLD_DIR", "data/gold")) / "step07_reflowed.json"
    )
    gp = Path(gold_path)
    if not gp.exists():
        print("SKIP: gold not found for step07")
        sys.exit(0)
    try:
        cand = _load_json(Path(cand_path))
        gold = _load_json(gp)
    except Exception as e:
        print("SKIP: step07 load unavailable", e)
        sys.exit(0)
    ...
    (OUT_JSON / "step_eval_step07.json").write_text(json.dumps(out, indent=2))
    print(f"Scenario pipeline/step07_eval_agent: {'pass' if passed else 'fail'}")
