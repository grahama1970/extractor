...
def _find_latest() -> Path | None:
    c = list(ROOT.glob("data/results/pipeline/**/06_figure_extractor/json_output/06_figures.json"))
...
def main():
    cand_path = os.getenv("STEP06_PATH") or (_find_latest() and str(_find_latest()))
    if not cand_path:
        print("SKIP: no STEP06_PATH and no discoverable candidate")
        sys.exit(0)
    gold_path = os.getenv("GOLD_PATH") or str(
        Path(os.getenv("GOLD_DIR", "data/gold")) / "step06_figures.json"
    )
    gp = Path(gold_path)
    if not gp.exists():
        print("SKIP: gold not found for step06")
        sys.exit(0)
    try:
        cand = _load_json(Path(cand_path))
        gold = _load_json(gp)
    except Exception as e:
        print("SKIP: step06 load unavailable", e)
        sys.exit(0)
    ...
    (OUT_JSON / "step_eval_step06.json").write_text(json.dumps(out, indent=2))
    print(f"Scenario pipeline/step06_eval_agent: {'pass' if passed else 'fail'}")
