...
def _call(model: str, prompt: str):
    try:
        ...
        out = litellm_call_sync(name='step06_eval', prompts=[prompt], models=[model], response_json=True, temperature=0, extra_kwargs={"max_tokens":512})
...
def main():
    ...
    if not cand_path:
        print('SKIP: no STEP06_PATH and no discoverable candidate'); sys.exit(0)
    gold_path = os.getenv('GOLD_PATH') or str(Path(os.getenv('GOLD_DIR','data/gold'))/ 'step06_figures.json')
    gp = Path(gold_path)
    if not gp.exists(): print('SKIP: gold not found for step06'); sys.exit(0)
    try: cand=_load_json(Path(cand_path)); gold=_load_json(gp)
    except Exception as e: print('SKIP: step06 load unavailable', e); sys.exit(0)
