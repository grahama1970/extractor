...
def _find_latest() -> Path | None:
    c = list(
        ROOT.glob("data/results/pipeline/**/09_section_summarizer/json_output/09_summaries.json")
    )
...
def main():
    ...
    if not cand_path:
        print("SKIP: no STEP09_PATH and no discoverable candidate")
        sys.exit(0)
    gold_path = os.getenv("GOLD_PATH") or str(
        Path(os.getenv("GOLD_DIR", "data/gold")) / "step09_summaries.json"
    )
    gp = Path(gold_path)
    if not gp.exists():
        print("SKIP: gold not found for step09")
        sys.exit(0)
    try:
        cand = _load_json(Path(cand_path))
        gold = _load_json(gp)
    except Exception as e:
        print("SKIP: step09 load unavailable", e)
        sys.exit(0)
