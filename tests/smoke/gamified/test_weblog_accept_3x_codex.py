...
def test_weblog_accept_3x_codex(tmp_path: Path):
    ...
    prompt.write_text('\n'.join([
        ...
        '## Codebase', 'repo_root: .',
        '## Approaches', '- name: mul_shift_add', '- name: mul_karatsuba', '- name: mul_chunked',
    ...
    rl = _http_post(f'{api_base}/ingest/log', {
        'ts': 0.0, 'run_id': rid, 'variant': 'acceptance_probe', 'episode_id': None,
        'stream': 'app', 'level': 'INFO', 'source': 'accept_smoke', 'message': 'probe', 'meta': {}
    }, timeout=5)
    assert json.loads(rl.read().decode('utf-8')).get('ok') is True
    re = _http_post(f'{api_base}/ingest/episode', {
        'ts': 0.0, 'run_id': rid, 'episode_id': 'probe-1', 'variant': 'acceptance_probe',
        'pass': True, 'score': 1.0, 'metrics': {}, 'error_count': 0, 'screenshots': []
    }, timeout=5)
