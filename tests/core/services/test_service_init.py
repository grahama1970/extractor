...
@pytest.mark.output_format("markdown")
@pytest.mark.config({"page_range": [0], "use_llm": True, "llm_service": "marker.services.litellm.LiteLLMService"})
...
def test_llm_default_is_litellm(pdf_converter: PdfConverter, temp_doc):
    # Test that LiteLLMService is now the default service
    assert pdf_converter.artifact_dict["llm_service"] is not None
    assert isinstance(pdf_converter.llm_service, LiteLLMService)