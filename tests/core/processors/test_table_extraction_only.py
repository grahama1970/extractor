"""
Simplified test script for table extraction functionality.

This script focuses only on testing table extraction without section hierarchy extraction.
"""
...
def main():
    ...
    provider = PdfProvider(filepath=fpath)
    
    # Create a document
    document = Document(filepath=fpath, pages=[])
    provider.fill_document(document)
    
    # Filter pages based on page_range
    if page_range:
        document.pages = [page for page in document.pages if page.page_id in page_range]
    ...
    for i, table in enumerate(tables):
        ...
        if table.metadata:
            for key, value in table.metadata.__dict__.items():
                if key.startswith("_"):
                    continue
                print(f"  {key}: {value}")
