...
Example usage:
    from marker.core.processors.enhanced_table import EnhancedTableProcessor
...
from marker.core.processors.table_optimizer import TableOptimizer, TableQualityEvaluator
from marker.core.schema import BlockTypes
from marker.core.schema.blocks import Block
from marker.core.schema.blocks.tablecell import TableCell
from marker.core.schema.document import Document
from marker.core.schema.groups.page import PageGroup
from marker.core.schema.polygon import PolygonBox
from marker.core.settings import settings
from marker.core.util import matrix_intersection_area
...
class EnhancedTableProcessor(BaseProcessor):
    ...
    def __init__(
        ...
        if hasattr(self.optimizer_config, 'enabled') and self.optimizer_config.enabled:
            self.optimizer = TableOptimizer(self.optimizer_config)
        else:
            self.optimizer = None
            
        if hasattr(self.quality_evaluator_config, 'enabled') and self.quality_evaluator_config.enabled:
    ...
    def parse_config(self):
        ...
        self.use_camelot_fallback = self.config.get("use_camelot_fallback", True)
        self.camelot_min_cell_threshold = getattr(self.camelot_config, 'min_cell_threshold', 4)
        ...
        self.camelot_copy_text = getattr(self.camelot_config, 'copy_text', False)
        ...
        self.camelot_column_tol = self.camelot_config.column_tol
            ...
            for block in page.contained_blocks(document, self.block_types):
                ...
                if cells:
                    ...
                    if self.quality_evaluator:
                        ...
                        # Get detailed quality metrics if available
                        if hasattr(self.quality_evaluator, 'get_detailed_metrics'):
                            quality_metrics = self.quality_evaluator.get_detailed_metrics()
                ...
                if self.use_camelot_fallback and CAMELOT_AVAILABLE and use_fallback:
                    ...
                    # Update table metadata for Surya extraction
                    if hasattr(block, 'update_extraction_metadata'):
                        block.update_extraction_metadata("surya", {
                            "model": "surya",
                            "method": "ml",
                            "detection_model": str(type(self.detection_model).__name__),
                            "recognition_model": str(type(self.recognition_model).__name__),
                            "table_rec_model": str(type(self.table_rec_model).__name__)
                        })
                    
                    # Update quality metrics if available
                    if quality_score is not None and hasattr(block, 'update_quality_metrics'):
                        block.update_quality_metrics(quality_score, quality_metrics)
            ...
            for table_info in fallback_tables:
                ...
                if optimized_params:
                    ...
                    if flavor == "lattice":
                        ...
                        camelot_kwargs.update({
                            ...
                            "column_tol": self.camelot_column_tol if hasattr(self, 'camelot_column_tol') else 0,
                        })
                ...
                try:
                    ...
                    if (len(tables) == 0 or tables[0].df.empty) and self.camelot_flavor == "auto":
                        ...
                        # Update parameters for the alternate flavor
                        if alt_flavor == "lattice":
                            # Remove stream-specific parameters
                            camelot_kwargs.pop("edge_tol", None)
                            camelot_kwargs.pop("row_tol", None)
                            camelot_kwargs.pop("column_tol", None)
                            # Add lattice-specific parameters
                            camelot_kwargs.update({
                                "line_scale": self.camelot_line_scale,
                                "line_width": self.camelot_line_width,
                            })
                        else:  # stream flavor
                            # Remove lattice-specific parameters
                            camelot_kwargs.pop("line_scale", None)
                            camelot_kwargs.pop("line_width", None)
                    ...
                    if len(tables) > 0 and not tables[0].df.empty:
                        ...
                        # Update extraction metadata with Camelot details
                        if hasattr(block, 'update_extraction_metadata'):
                            extraction_details = {
                                "model": "camelot",
                                "method": "heuristic",
                                "flavor": camelot_kwargs.get("flavor", "lattice"),
                                "line_scale": camelot_kwargs.get("line_scale", self.camelot_line_scale),
                                "line_width": camelot_kwargs.get("line_width", self.camelot_line_width),
                                "edge_tol": camelot_kwargs.get("edge_tol", self.camelot_edge_tol),
                                "row_tol": camelot_kwargs.get("row_tol", self.camelot_row_tol),
                                "copy_text": camelot_kwargs.get("copy_text", self.camelot_copy_text),
                                "optimized": optimized_params is not None
                            }
                            if camelot_kwargs.get("flavor") == "stream":
                                extraction_details["row_tol"] = camelot_kwargs.get("row_tol", 2)
                                extraction_details["column_tol"] = camelot_kwargs.get("column_tol", 0)
                            
                            block.update_extraction_metadata("camelot", extraction_details)
    ...
    def _merge_table_run(self, document: Document, blocks: List[Block]):
        ...
        # Capture original table data before merging
        for block in blocks:
            table_data = {
                "id": f"table_page{block.page_id}_{block.id.block_idx}",
                ...
                "extraction_details": getattr(block, 'extraction_details', None),
        ...
        for i in range(1, len(blocks)):
            ...
            merge_method = "heuristic"
            merge_confidence = 0.8  # Default confidence for heuristic
        ...
        # Update merge information on the final merged table
        if hasattr(start_block, 'update_merge_info') and len(blocks) > 1:
            start_block.update_merge_info(
