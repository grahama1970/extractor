...
def _flatten_unified(unified_doc: Dict[str, Any], source: Path, target_root: Path) -> Path:
    ...
    This is a simplified flattening for structured formats (HTML, DOCX, etc.)
    that don't go through the full pipeline. For PDFs, use s10_arangodb_exporter.
    """
    import hashlib
    ...
    for i, block in enumerate(blocks):
        ...
        # Handle different block content types
        if isinstance(content, dict):
            text_content = content.get("text", "") or json.dumps(content)
        elif isinstance(content, list):
            # Table rows or similar
            text_content = json.dumps(content)
        else:
            text_content = str(content) if content else ""

        entry = {
            "_key": hashlib.md5(f"{doc_key}_{block_id}".encode()).hexdigest(),
