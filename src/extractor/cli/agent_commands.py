...
# Constants
ARANGODB_PATH = "."
MARKER_MESSAGES_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "messages"
)
...
def process_message(
    ...
    response = {
        ...
        "metadata": {
            ...
            "pdf_export_info": {
                ...
                "renderer_formats": ["ArangoDB format", "JSONL format", "Markdown format"],
            },
        },
    }
    ...
    response_file = os.path.join(
        ARANGODB_MESSAGES_DIR, "from_marker", f"response_{int(time.time())}.json"
    )
    try:
    ...
    try:
        subprocess.run(
            [
                ...
                "&&",
                "python",
                "src/arangodb/cli/agent_commands.py",
                "process-message",
                response_file,
            ],
            shell=True,
...
def send_message(
    ...
    message_type: str = typer.Option(
        "request", help="Message type (request/response/notification)"
    ),
):
    """
    ...
    if message_type not in valid_types:
        typer.echo(
            f"Error: Invalid message type: {message_type}. Valid types: {', '.join(valid_types)}"
        )
        raise typer.Exit(code=1)
    ...
    message_file = os.path.join(
        ARANGODB_MESSAGES_DIR, "from_marker", f"message_{int(time.time())}.json"
    )
    try:
