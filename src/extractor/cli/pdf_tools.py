...
def snapshot(
    ...
    region = {
        "page": page,
        "bbox": [x0, y0, x1, y1],
        "label": label or f"Page {page} region"
    }
...
def list_commands():
    ...
    typer.echo("   python pdf_tools.py snapshot doc.pdf --page 0 --x0 100 --y0 200 --x1 500 --y1 400\n")
    
    typer.echo("2. snapshot-multi - Extract multiple regions")
    typer.echo("   python pdf_tools.py snapshot-multi doc.pdf '[{\"page\":0,\"bbox\":[100,200,500,400]}]'\n")
