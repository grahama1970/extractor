...
async def extract_to_unified_json(
    ...
    try:
            ...
            if annot_config and annot_config.enabled:
                ...
                if annot_result["success"]:
                    ...
                    if require_gold_standard_validation:
                        ...
                        logger.debug(f"Stage 1 Debug: Extracted {len(annot_result.get('annotations', []))} annotations")
                        for i, annot in enumerate(annot_result.get('annotations', [])[:3]):
                            logger.debug(f"  Annotation {i}: {annot.get('type', 'Unknown')} - '{annot.get('content', '')}'")
                        ...
                        if similarity < 0.9:
                            logger.warning(f"Stage 1 validation: annotation similarity {similarity*100:.1f}% below 90%")
                            logger.debug(f"Gold standard expects: {stage1_result.get('gold_count', 0)} annotations")
                            logger.debug(f"We extracted: {stage1_result.get('extracted_count', 0)} annotations")
                            # Don't fail on Stage 1 - annotations are supplementary
                        else:
                            logger.success(f"✅ Stage 1 validation PASSED: annotation similarity {similarity*100:.1f}% >= 90%")
                        
                        validation_results["stage1"] = stage1_result
                    
                    # Save if configured
                    if annot_config.settings.get("save_to_file"):
                        output_path = Path(annot_config.settings.get("output_path", "tmp/annotations.json"))
        ...
        config_dict = {
            "use_llm": use_llm,
            "output_format": "json",
            "llm_service": "extractor.core.services.marker_litellm_adapter.create_marker_compatible_litellm_service" if use_llm else None,
        ...
        if processors is None:
            ...
            default_processors = [p.__module__ + "." + p.__name__ for p in MarkerPdfConverter.default_processors]
            ...
            for i, p in enumerate(default_processors):
                ...
                if "SectionHeaderProcessor" in p:
                    ...
                    enhanced_processors.append("extractor.core.processors.blockquote.BlockquoteProcessor")
                elif "FootnoteProcessor" in p:
                    # Use our enhanced footnote processor
                    enhanced_processors.append("extractor.core.processors.footnote.FootnoteProcessor")
                else:
            ...
            for p in enhanced_processors:
                if "extractor.core.processors.sectionheader.SectionHeaderProcessor" in p or \
                   "extractor.core.processors.knowledge_aware_sectionheader.KnowledgeAwareSectionHeaderProcessor" in p:
        ...
        if 'tasks' in config and (config['tasks'] is None or 
                                  (isinstance(config['tasks'], list) and all(t is None for t in config['tasks']))):
            ...
            try:
                ...
                class DictWrapper:
                    def __init__(self, d):
                        ...
                        if 'children' in d:
                            self.children = [DictWrapper(child) if isinstance(child, dict) else child for child in d['children']]
        ...
        if hasattr(rendered, 'children') and rendered.children:
            ...
            for i, page_child in enumerate(rendered.children):
                if hasattr(page_child, 'block_type') and page_child.block_type == "Page":
                    logger.info(f"Page {i}: has {len(getattr(page_child, 'children', []))} children")
        ...
        def extract_blocks_recursive(node, parent_text=''):
            ...
            if hasattr(node, 'block_type'):
                ...
                if node.block_type == "SectionHeader":
                    ...
                    logger.debug(f"  Attributes: {[attr for attr in dir(node) if not attr.startswith('_')]}")
            ...
            for page_idx, page in enumerate(rendered.children):
                    ...
                    if hasattr(page, 'children'):
                        for child_idx, child in enumerate(page.children):
                            if hasattr(child, 'text'):
                                logger.debug(f"Page 1 child {child_idx}: text='{getattr(child, 'text', '')[:50]}...'")
                ...
                def extract_page_blocks(node, parent_text=''):
                    ...
                    if hasattr(node, 'block_type'):
                        ...
                        # Debug logging for section headers
                        if node.block_type == "SectionHeader":
                            logger.debug(f"Found SectionHeader on page {current_page}: text='{text}'")
                        ...
                        if hasattr(node, 'metadata') and node.metadata:
                            ...
                            if hasattr(metadata, 'validation'):
                                ...
                                block["suspicious_reason"] = getattr(validation, 'suspicious_reason', None)
        ...
        if require_gold_standard_validation and validator and all_blocks:
            ...
            with open(stage2_blocks_path, 'w') as f:
                json.dump({
                    "metadata": {
                        ...
                        "block_count": len(all_blocks),
                        "processors_applied": ["SectionHeaderProcessor", "ListProcessor", "TableProcessor"]
                    },
                    "blocks": all_blocks
                }, f, indent=2)
            ...
            if stage2_result.get("validated") and "average_text_similarity" in stage2_result:
                if stage2_result["average_text_similarity"] < 0.9:
                    logger.warning(f"Stage 2 validation: raw block similarity {stage2_result['average_text_similarity']:.2f} below 90%")
                    logger.info(f"Note: {stage2_result['block_count']} raw blocks will be processed to {stage2_result['gold_count']} final blocks")
        ...
        section_pattern = re.compile(r'^\s*(\d+(?:\.\d+)*\.?)\s+(.+?)(?:\s+submodule)?$', re.MULTILINE)
            ...
            if block["block_type"] == "Text":
                ...
                if match:
                    ...
                    if section_num.startswith("4.1.5"):
                        logger.info(f"Found section header in text block: {section_num} {section_title}")
        ...
        if use_llm and sections:
            ...
            if empty_headers:
                logger.info(f"Found {len(empty_headers)} empty section headers, using LLM to enhance")
        ...
        if failed_tables:
            ...
            for ft in failed_tables:
                logger.debug(f"Failed table: text='{ft.get('text', '')[:50]}', html='{ft.get('html', '')[:50]}'")
        ...
        if pipeline_config:
            ...
            for processor_config in pipeline_config.get_enabled_processors():
                ...
                try:
                    ...
                    if processor:
                        logger.info(f"Applying processor: {processor_config.name} ({processor_config.type.value})")
                        ...
                        if processor_config.type == ProcessorType.TEXT_CLEANING:
                            ...
                            from extractor.core.processors.section_metadata_propagator import organize_blocks_into_sections
                            ...
                            if 'section_structure' not in result:
                                logger.warning("No section structure found, skipping semantic processing")
                                continue
            ...
            if False:
                try:
                    ...
                    logger.info(f"Header validation - Before: {stats_before['accuracy']:.1f}%, After: {stats_after['accuracy']:.1f}%")
            ...
            try:
                ...
                if annotations:
                    result['annotations'] = annotations
                    logger.info(f"Added {len(annotations.get('annotations', []))} annotations to result")
                ...
                if require_gold_standard_validation and validator:
                    ...
                    if not validator.require_minimum_score(0.9):
                        logger.error("Stage 3 validation failed - section structure quality below 90%")
                        if fail_on_validation_error:
                            raise ValueError("Stage 3 validation failed - quality below requirement")
        ...
        if use_llm and annotations and "annotations" in annotations:
            ...
            try:
                from extractor.core.processors.llm.llm_annotation_interpreter import enhance_with_annotations
                ...
                if enhanced_result:
                    ...
                    logger.success("Stage 5 completed: Document enhanced with annotation interpretations")
                    ...
                    if "metadata" in result and "annotation_interpretation" in result["metadata"]:
                        ...
                        logger.info(f"Enhancement stats: {stats['interpretations_count']} interpretations, "
        ...
        if require_gold_standard_validation and validator and use_llm and annotations:
            try:
                ...
                if stage5_result.get("has_enhancements"):
                    logger.success(f"Stage 5 enhancements applied: {stage5_result['enhanced_blocks_count']} blocks enhanced")
                else:
...
async def working_usage() -> bool:
    ...
    try:
        ...
        for i, block in enumerate(page1_blocks):
            logger.debug(f"Page 1 block {i}: type={block['block_type']}, text preview='{block.get('text', '')[:50]}...'")
        ...
        for i, block in enumerate(extracted_blocks):
            ...
            if "cv32a65x" in text.lower() or "Due to" in text:
                ...
                logger.info(f"Found cv32a65x/Due to text in block {i} (page {block.get('page', 0)}, type {block['block_type']}): '{text[:100]}...'")
        ...
        if not cv32_text_found:
            ...
            table_indices = [i for i, b in enumerate(extracted_blocks) if b["block_type"] == "Table"]
            if table_indices:
                last_table_idx = table_indices[-1]
                logger.info(f"Last table at index {last_table_idx}, checking text blocks after it...")
        ...
        if gold_standard_path.exists():
            ...
            if section_headers:
                ...
                if "BHT" in header_text and "Branch History Table" in header_text:
                    ...
                    logger.warning(f"✗ Section header mismatch. Got: '{header_text}', Expected: '{expected_text}'")
                total_checks += 1
            ...
            logger.info(f"Checking {len(text_blocks)} text blocks against {len(expected_texts)} expected")
            ...
            for i, expected in enumerate(expected_texts[:3]):  # Check first 3 text blocks
                ...
                for phrase in key_phrases:
                    ...
                    if len(phrase) > 20:  # Meaningful phrase
                        ...
                        found = any(normalized_phrase in b["text"].replace("_", " ") for b in text_blocks)
                        if found:
                            content_matches += 1
            ...
            logger.info(f"Content match rate: {overall_match_rate:.1f}% ({content_matches}/{total_checks})")
            logger.info(f"Block type match rate: {type_match_rate:.1f}%")
            ...
            assert combined_score >= 90, f"Gold standard match {combined_score:.1f}% is below 90% requirement"
        ...
        gold_format = {
            ...
            "summary": {
                ...
                "section_headers": len([b for b in extracted_blocks if b["block_type"] == "SectionHeader"])
            }
        }
        ...
        if PYMUPDF_AVAILABLE:
            ...
            pymupdf_result = await extract_to_unified_json(str(test_pdf), use_llm=False, use_pymupdf=True)
