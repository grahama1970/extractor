...
Module: table_quality_metrics.py
Description: Functions for table quality metrics operations
        ...
        if numeric_count / len(df) > 0.5:
            ...
            for value in df[col]:
                if not (
                    ...
                    or (isinstance(value, str) and value.strip().replace(".", "", 1).isdigit())
...
def calculate_consistency_score(df: pd.DataFrame) -> float:
    ...
    consistency_score = column_weight * int(consistent_columns) + data_type_weight * int(
...
if __name__ == "__main__":
        ...
        if not found_table:
            ...
            if not (0 <= confidence["confidence"] <= 100):
                all_validation_failures.append(
                    f"Confidence score out of range: {confidence['confidence']}"
                )
    ...
    try:
        ...
        if completeness != 0:
            all_validation_failures.append(
                f"Completeness for empty DataFrame should be 0, got {completeness}"
            )
