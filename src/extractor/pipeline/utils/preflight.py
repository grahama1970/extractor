#!/usr/bin/env python3
"""Environment and connectivity preflight checks for SciLLM/Chutes API availability."""
...
def scillm_quick_check(timeout: float = 3.0) -> Tuple[bool, str]:
    ...
    - Verifies SCILLM_API_BASE and SCILLM_PROXY_KEY are set
    - Makes a cheap GET to /v1/models (or /models if base already endswith /v1) with x-api-key
    - Returns (ok, reason)
    """
    base = (os.getenv("SCILLM_API_BASE", "http://localhost:4010") or "").rstrip("/")
    key = os.getenv("SCILLM_PROXY_KEY", "sk-dev-proxy-123") or ""
    if not base or not key:
        return False, "SCILLM_API_BASE/SCILLM_PROXY_KEY not set"
    try:
...
def camelot_quick_check() -> Tuple[bool, str]:
    """Check if Camelot is installed and accessible."""
    try:
