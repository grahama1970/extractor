...
async def preflight_vision_support(model: str, timeout_sec: int = 10) -> bool:
    ...
    try:
        ...
        use_remote = os.getenv('VISION_PREFLIGHT_USE_REMOTE', '').lower() in ('1','true','yes','y')
        image_part: Dict[str, Any]
        if use_remote:
            image_part = {'type': 'image_url', 'image_url': {'url': 'https://httpbin.org/image/png'}}
        else:
            size = 256
            try:
            ...
            if _HAS_PIL and size >= 1:
                ...
                try:
                    ...
                    image_part = {'type': 'image_url', 'image_url': {'url': f'data:image/png;base64,{_TINY_PNG_B64}'}}
            else:
                # Fallback to tiny PNG if PIL is unavailable
                image_part = {'type': 'image_url', 'image_url': {'url': f'data:image/png;base64,{_TINY_PNG_B64}'}}

        # Gemini expects input_text/input_image parts; others accept text/image_url
        is_gemini = _norm_model(model).startswith('gemini/') or 'gemini' in _norm_model(model)
        content_parts = [
            ({'type': 'input_text', 'text': 'preflight vision capability'} if is_gemini else {'type': 'text', 'text': 'preflight vision capability'}),
        ...
        ret = await litellm_call([params], wrap_json=False, concurrency=1, desc='Vision Preflight')
        # Consider an empty response as failure (some providers return empty on unsupported input)
        if not ret or not isinstance(ret[0], str) or not ret[0].strip():
