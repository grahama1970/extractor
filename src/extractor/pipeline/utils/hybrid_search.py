...
def hybrid_search_annotations(DB: Any, available: bool, section_text: str, page_num: int, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        ann_coll_candidates = ([env_ann] if env_ann else []) + ["annotations", "pdf_annotations", "doc_annotations"]
        edge_coll_candidates = ([env_edge] if env_edge else []) + ["annotation_edges", "edges", "relationships"]
        ...
        def _k(x: Dict[str, Any]) -> str:
            return str(x.get("_id") or x.get("_key") or x.get("id") or hash(json.dumps(x, sort_keys=True)))
