# Review: Contract Loop (Strict Step-by-Step Convergence)

## Repository and branch

- **Repo:** `grahama1970/extractor`
- **Branch:** `feature/contract-loop`
- **Paths of interest:**
  - `tools/contract_loop/docs/CONTRACT_LOOP.md`
  - `tools/contract_loop/verify_pipeline_contract.py`
  - `tools/contract_loop/adapters/extractor/fixtures/BHT_CV32A65X_with_requirements_noannots.json`
  - `tools/contract_loop/judges/llm_judge.schema.json`
  - `tools/contract_loop/adapters/extractor/docs/CONTRACT.md`

## Summary

We are adopting a **strict, step-by-step Contract Loop** (a contract-driven fork of the Ralph Wiggum loop) that:
...
### 1. Code Review: Contract Loop Enforcement

In `tools/contract_loop/verify_pipeline_contract.py` (wrapper: `scripts/verify_pipeline_contract.py`):
...
In `tools/contract_loop/adapters/extractor/docs/CONTRACT.md`:
...
- `python scripts/verify_pipeline_contract.py --mode deterministic --fixture tools/contract_loop/adapters/extractor/fixtures/BHT_CV32A65X_with_requirements_noannots.json --pdf data/input/pipeline/BHT_CV32A65X_with_requirements_noannots.pdf`
  - Reruns upstream steps and deletes downstream outputs per attempt.
  - Stops on the first failing step.
- `python scripts/verify_pipeline_contract.py --mode full --llm-judge --skip-lean4 --fixture tools/contract_loop/adapters/extractor/fixtures/BHT_CV32A65X_with_requirements_noannots.json --pdf data/input/pipeline/BHT_CV32A65X_with_requirements_noannots.pdf`
