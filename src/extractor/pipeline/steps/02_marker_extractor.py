...
console = Console()
app = typer.Typer(help="Stage-02: native JSON block extractor")
                ...
                if block.block_type.name in ['SectionHeader', 'Text', 'Table', 'Figure', 'ListItem']:
                    ...
                    try:
                        ...
                        if spans:
                            ...
                            try:
                                font_size = float(font_size_val) if font_size_val is not None else None
                            except Exception:
                                font_size = None
                            ...
                            if fitz_doc is not None and hasattr(block, 'polygon') and getattr(block, 'polygon'):
                                try:
                                    ...
                                    if bbox:
                                        ...
                                        if found_color is not None:
                                            ...
                                            try:
                                                ...
                                                first_span_font["color_hex"] = f"#{r:02X}{g:02X}{b:02X}"
...
def run(
    ...
    output_dir: Path = typer.Option("data/results/pipeline", "--output-dir", "-o", help="Parent directory for pipeline results."),
    timeout: int = typer.Option(300, "--timeout", help="Max seconds allowed for extraction before timing out."),
    debug: bool = typer.Option(False, "--debug", help="Enable verbose logging to a stage log file."),
    no_spawn: bool = typer.Option(False, "--no-spawn/--spawn", help="Run extraction inline (no subprocess, easier debugging)."),
    ...
    try:
        if sampler and not gpu_metrics_available():
            diagnostics.append(make_event("02_marker_extractor","info","gpu_metrics_unavailable","NVML not available; GPU metrics disabled",{}))
    except Exception:
        pass
    try:
    ...
    if no_spawn:
        ...
        if p.is_alive():
            console.print(f"[red]Stage 02 timed out after {timeout}s. Terminating extractor...[/red]")
            try:
                diagnostics.append(make_event("02_marker_extractor","error","extractor_timeout", f"Timed out after {timeout}s", {"pdf_path": str(pdf_path), "timeout": timeout}))
                errors_count += 1
            except Exception:
                pass
            try:
        ...
        if not result.get("ok", False):
            ...
            try:
                diagnostics.append(make_event("02_marker_extractor","error","extractor_process_error", str(result.get("error", "Unknown error")), {"pdf_path": str(pdf_path)}))
                errors_count += 1
            except Exception:
                pass
    ...
    if output_suffix:
        safe = "".join(ch if ch.isalnum() or ch in ("-", "_") else "_" for ch in output_suffix.strip())
        if safe:
            base = f"{base}_{safe}"
    ...
    if suspicious_blocks:
        console.print(f"⚠️  Found {len(suspicious_blocks)} suspicious blocks for Stage 03 verification.")
...
def debug_bundle(
    bundle: Path = typer.Argument(..., exists=True, file_okay=True, dir_okay=False, readable=True, help="Bundle JSON with key 'clean_pdf'"),
    ...
    try:
        ...
        typer.secho(f"Extraction failed: {e}", fg=typer.colors.RED); raise typer.Exit(1)
...
if __name__ == "__main__":
    if DEBUG:
        # For debug mode, use a test PDF
        test_pdf = Path("data/results/pipeline/01_annotation_processor/BHT_CV32A65X_marked_clean.pdf")
        if not test_pdf.exists():
            console.print(f"[red]Debug mode: Cleaned PDF not found at {test_pdf}[/red]")
            raise SystemExit(1)
        run(test_pdf)
    else:
        app()
