...
class DeterministicValidator:
    ...
    RESOLVED_ORDER = [
        ...
        {"page": 1, "y": 323, "type": "figure", "content": "(BHT state machine diagram)"},
        {
            ...
            "y": 324,
            "type": "text",
            "content": "When a branch instruction is pre-decoded by instr_scan submodule, the BHT valids whether the PC address is in the BHT and provides the taken or not prediction. The BHT is never flushed.",
        },
        # Page 2
        {
            "page": 2,
    ...
    def validate_deterministic_obj_match(self, obj: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for expected in self.RESOLVED_ORDER:
            if (
                expected["page"] == page
                and abs(expected["y"] - y_pos) <= 3  # ±3 pixels tolerance
                and expected["type"] == obj_type
            ):
        ...
        return {
            ...
            "content_similarity": (
                self._fuzzy_content_match(matched["content"], content, 0.8) if matched else 0
            ),
...
class LLMStreamGenerator:
    ...
    def generate_base_stream(self, extraction_data: Dict[str, Any]) -> str:
        ...
        stream.append(
            "\u003e **Reliability**: ✅ Validated against resolved PDF object order\u003e"
        )
        ...
        for page in sorted(pages.keys()):
            ...
            for obj in pages[page]:
                ...
                if obj_type == "section":
                    stream.append(f"\n## {content} [Y:{y_pos}]")
                    stream.append(
                        f"<!-- DETERMINISTIC: {validation['status']} (ΔY={validation.get('delta_y', '?')}) -->"
                    )
...
def main():
    ...
    parser.add_argument(
        "-o", "--output", default="llm_readable_stream.md", help="Output stream file"
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
    ...
    try:
        ...
        print(
            f"Deterministic validation: {total_count - failed_count}/{total_count} objects matched"
        )

        if failed_count > 3:  # Allow up to 3 minor mismatches
            print(
                f"ERROR: Too many validation failures ({failed_count}), extraction not deterministic enough"
            )
            sys.exit(1)
