...
from extractor.pipeline.utils.diagnostics import start_resource_sampler, stop_resource_sampler, make_event, snapshot_resources, build_stage_timings, get_run_id, gpu_metrics_available
import textwrap
...
# Initialize
app = typer.Typer(help="Build sections with sophisticated header detection")
console = Console()
...
def detect_header_level(text: str) -> int:
    ...
    # Level 1 keywords
    if any(keyword in lower_text for keyword in ['introduction', 'abstract', 'conclusion', 'references', 'appendix']):
        return 1
    
    # Level 2 keywords  
    if any(keyword in lower_text for keyword in ['methodology', 'implementation', 'results', 'discussion']):
        return 2
    
    # Level 3 keywords
    if any(keyword in lower_text for keyword in ['interface', 'protocol', 'algorithm', 'structure']):
        return 3
    
    # Default to level 2
    return 2

def build_sections_from_blocks(blocks: List[Dict[str, Any]], fallback_heuristics: bool = False) -> List[Dict[str, Any]]:
    ...
    for block in blocks:
        ...
        if block_type == "SectionHeader":
            lv = (block.get('llm_verification') or {}).get('result') if isinstance(block.get('llm_verification'), dict) else None
            ...
            if isinstance(lv, dict) and 'is_header' in lv:
                ...
                accepted = bool(na.get('has_numbering') or (is_bold and (font_size or 0) >= LARGE_FONT_THRESHOLD))
            else:
                accepted = True
            ...
            if accepted:
                ...
                try:
                    import hashlib
                    sec_hash = hashlib.md5((na.get('title_text') or section_title or clean_title).lstrip('. ').strip().encode('utf-8')).hexdigest()
                except Exception:
                    sec_hash = ''
                ...
                if current_section:
                    ...
                    current_section = {
                        ...
                        "bbox": block.get("bbox", [0, 0, 100, 100]),
                        "metadata": {"block_count": 1, "auto_generated": True, "reason": "not_accepted_as_header"},
                    }
        elif current_section:
            current_section["blocks"].append(block)
            current_section["metadata"]["block_count"] += 1
            current_section["page_end"] = max(current_section["page_end"], block.get("page", block.get("page_idx", 0)))
            # Expand bbox
            if "bbox" in block:
                cb = current_section["bbox"]
                bb = block["bbox"]
                current_section["bbox"] = [min(cb[0], bb[0]), min(cb[1], bb[1]), max(cb[2], bb[2]), max(cb[3], bb[3])]
            # Enrich
            try:
    ...
    for i, section in enumerate(sections):
        ...
        try:
            ps = int(section.get('page_start', 0)); pe = int(section.get('page_end', ps))
...
def find_parent_section_advanced(previous_sections: List[Dict], current_level: int) -> Optional[str]:
...
def extract_section_visual_enhanced(pdf_path: Path, section: Dict[str, Any], output_path: Path, expand: float = 0.3, max_pages: int = MAX_VISUAL_PAGES_DEFAULT) -> Optional[str]:
    ...
    try:
        ...
        if max_pages > 0 and len(page_range) > max_pages:
            ...
            try:
                ...
                _append_diag(section, "info", "composite_capped", f"Composite capped to {len(page_range)} pages", {"pages_included": page_range, "extra_pages": extra_pages})
            except Exception:
                pass
        ...
        total_height = sum(img.height for img in page_images) + page_break_height * (len(page_images) - 1)
        ...
        for i, img in enumerate(page_images):
            ...
            if i < len(page_images) - 1:
                draw.line(
                    [(0, y_offset), (max_width, y_offset)],
                    fill='red',
                    width=page_break_height
                )
        ...
        with BytesIO() as buf:
            ...
            try:
                section.setdefault('metadata', {})['composite_size_bytes'] = int(output_path.stat().st_size) if output_path.exists() else None
        ...
        try:
            _append_diag(section, "error", "visual_extract_failed", str(e), {"section_id": section.get("id")})
        except Exception:
            pass
        return None
...
def summarize_suspicious_from_verified(blocks: list[dict], sections: list[dict]) -> dict:
    ...
    false_pos = []
    for b in blocks:
        lv = (b.get('llm_verification') or {}).get('result') if isinstance(b.get('llm_verification'), dict) else None
...
async def process_sections_comprehensive(
    ...
    if pdf_path and pdf_path.exists() and image_output_dir:
        ...
        for section in sections:
            ...
            visual_b64 = extract_section_visual_enhanced(pdf_path, section, visual_path, expand=0.3, max_pages=max_visual_pages)
            if visual_b64:
    ...
    return {
        ...
        "statistics": {
            ...
            "validation_rate": suspicious_analysis["validated_sections"] / len(sections) if sections else 0.0,
            "suspicious_rate": suspicious_analysis["statistics"]["validation_summary"].get("suspicious_rate", 0.0)
        }
    }
...
async def build_and_validate_sections_comprehensive(
    ...
    try:
        if sampler and not gpu_metrics_available():
            diagnostics.append(make_event("04_section_builder","info","gpu_metrics_unavailable","NVML not available; GPU metrics disabled",{}))
    except Exception:
        pass
    ...
    section_result = await process_sections_comprehensive(blocks, pdf_path, image_output_dir, fallback_heuristics=fallback_heuristics, max_visual_pages=max_visual_pages)
...
def run(
    ...
    pdf_dir: Path = typer.Option(..., "--pdf-dir", help="Directory with the clean PDF from Stage 01."),
    ...
    fallback_heuristics: bool = typer.Option(False, "--fallback-heuristics/--no-fallback-heuristics", help="Enable minimal header detection if Stage 03 metadata is missing"),
    max_visual_pages: int = typer.Option(MAX_VISUAL_PAGES_DEFAULT, "--max-visual-pages", help="Max pages to include in a section composite image"),
):
    ...
    try:
        ...
        _lg.remove()
        _lg.add(str(stage_output_dir / "stage_04_section_builder.log"),
                level="DEBUG" if debug else "INFO", enqueue=True, backtrace=True, diagnose=False,
    ...
    output_path, result = asyncio.run(
        build_and_validate_sections_comprehensive(input_json, pdf_path, stage_output_dir, fallback_heuristics=fallback_heuristics, max_visual_pages=max_visual_pages)
    )
    ...
    if result.get("success"):
        ...
        console.print(f"❌ Section building failed.")

@app.command("debug-bundle")
def debug_bundle(
    bundle: Path = typer.Argument(..., exists=True, file_okay=True, dir_okay=False, readable=True, help="Bundle with keys: verified_blocks (Stage 03 object), clean_pdf (path)"),
    output_dir: Path = typer.Option(..., "-o", help="Parent directory for pipeline results."),
    ...
    try:
        ...
        output_path, result = asyncio.run(build_and_validate_sections_comprehensive(tmp_json, Path(clean_pdf), stage_output_dir, fallback_heuristics=fallback_heuristics, max_visual_pages=max_visual_pages))
        if result.get('success'):
            console.print(f"✅ Debug bundle sections built: {output_path}")
        else:
            typer.secho("Debug bundle section build failed.", fg=typer.colors.RED); raise typer.Exit(1)
    except Exception as e:
        typer.secho(f"Failed to run debug-bundle: {e}", fg=typer.colors.RED); raise typer.Exit(1)
