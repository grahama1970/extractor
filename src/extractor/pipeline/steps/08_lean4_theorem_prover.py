...
# LLM Configuration
LEAN4_MODEL = os.getenv("LEAN4_MODEL", "openai/gpt-5-mini")  # Fast, cost-effective model for extraction
MAX_CONCURRENT_LLM = int(os.getenv("MAX_CONCURRENT_LLM_CALLS", 5))
MAX_CONCURRENT_LEAN4 = int(os.getenv("MAX_CONCURRENT_LEAN4_CALLS", 2))  # Lean 4 is heavy (30-300s per theorem)
...
async def identify_requirements_in_section(
    ...
    async with semaphore:
        try:
            ...
            params: Dict[str, Any] = {
                "model": LEAN4_MODEL,
                "messages": [{"role": "system", "content": JSON_SYSTEM_GUARD}, {"role": "user", "content": prompt}],
                "timeout": 120,
            ...
            if not isinstance(content, str) or not content.strip():
                logger.warning("Requirement extraction returned empty content; defaulting to empty lists.")
                return [], []
            ...
            logger.info(f"Section '{section.get('title')}': Found {len(requirements)} requirements, {len(constraints)} constraints")
            
            return requirements, constraints
            
        except Exception as e:
            logger.error(f"Failed to extract requirements from section '{section.get('title', 'Unknown')}': {e}")
...
async def _prove_via_cli(requirement: str, strategy: Any) -> Dict[str, Any]:
    ...
    if "{stdin}" in LEAN4_CLI_CMD:
        ...
        try:
            ...
            return {"success": False, "stderr": f"CLI invoke failed: {e}", "return_code": 1, "lean_code": "", "stdout": ""}
    ...
    if "{input}" in LEAN4_CLI_CMD and "{output}" in LEAN4_CLI_CMD:
        with tempfile.TemporaryDirectory() as td:
            ...
            cmd_str = LEAN4_CLI_CMD.replace("{input}", str(in_path)).replace("{output}", str(out_path))
            argv = shlex.split(cmd_str)
            try:
                proc = await asyncio.create_subprocess_exec(
                    *argv,
                ...
                try:
                    ...
                    result = {"success": False, "stderr": "Output file missing or invalid JSON", "stdout": stdout.decode("utf-8", errors="ignore")}
                return {
                    "success": bool(result.get("success", False)),
                    "lean_code": result.get("lean_code", ""),
    ...
    return {"success": False, "stderr": "LEAN4_CLI_CMD missing required placeholders ({stdin} or {input}/{output})", "return_code": 1, "lean_code": "", "stdout": ""}
    ...
    try:
        ...
        if "{input_jsonl}" in LEAN4_CLI_CMD and "{output_jsonl}" in LEAN4_CLI_CMD:
            with tempfile.TemporaryDirectory() as td:
                ...
                cmd_str = (
                    LEAN4_CLI_CMD
                    .replace("{input_jsonl}", str(in_path))
        ...
        if "{input_json}" in LEAN4_CLI_CMD and "{output_json}" in LEAN4_CLI_CMD:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                cmd_str = (
                    LEAN4_CLI_CMD
                    .replace("{input_json}", str(in_json))
                ...
                if out_json.exists():
                    ...
                    results.append({"success": False, "stderr": stdout.decode("utf-8", errors="ignore") or "Output file missing"})
                return results
    except Exception as e:
...
async def prove_requirement(requirement: str, strategy: Any):
    ...
    try:
        ...
        from types import SimpleNamespace
        return SimpleNamespace(success=False, lean_code="", stdout="", stderr=f"generate_lean_code unavailable: {e}", return_code=1, test_filename="<stdin>", error_messages=[str(e)], proof_output=None)
...
async def prove_with_feedback(item: Dict[str, Any], item_type: str, semaphore: asyncio.Semaphore) -> Dict[str, Any]:
    """
    ...
    async with semaphore:
        try:
            ...
            if item_type == 'requirement':
                ...
                try:
                    ...
                    strategy = SimpleNamespace(
                        validation_approach="direct proof",
                        key_properties=[],
                        dependencies=[]
                    )
            ...
            if success_check:
                ...
                if item_type == 'requirement' and hasattr(result, 'error_messages') and result.error_messages:
                ...
                return {
                    ...
                    'advice': proof_dict.get('advice', 'Check theorem syntax and try simplifying the statement'),
...
async def process_reflowed_sections(pipeline_data: Dict[str, Any], skip_proving: bool = False) -> Dict[str, Any]:
    """
    ...
    for coro in tqdm(asyncio.as_completed(extraction_tasks), total=len(extraction_tasks), desc="Extracting Requirements"):
        requirements, constraints = await coro
        all_requirements.extend(requirements)
        all_constraints.extend(constraints)

    logger.info(f"Extracted {len(all_requirements)} requirements and {len(all_constraints)} constraints.")

    if skip_proving:
        logger.info("Skipping Lean 4 proving as requested.")
        return {
            'success': True,
            'statistics': {'total_requirements_found': len(all_requirements), 'total_constraints_found': len(all_constraints)},
            'proof_results': []
        }
    ...
    if LEAN4_CLI_CMD and (("{stdin_jsonl}" in LEAN4_CLI_CMD) or ("{input_jsonl}" in LEAN4_CLI_CMD and "{output_jsonl}" in LEAN4_CLI_CMD) or ("{input_json}" in LEAN4_CLI_CMD and "{output_json}" in LEAN4_CLI_CMD)):
        try:
            ...
            for r in batch_out or []:
                ...
                out_entry: Dict[str, Any] = {
                    ...
                    "duration_seconds": float(r.get("duration_seconds", 0)) if isinstance(r.get("duration_seconds", 0), (int, float)) else 0.0,
                    "timestamp": datetime.now().isoformat()
                }
                if not success:
                    out_entry.update({
                        "error": r.get("stderr", "") or "Unknown error",
                        "advice": r.get("advice", "Check theorem syntax and try simplifying the statement"),
    ...
    proof_tasks = [prove_with_feedback(item['item'], item['type'], lean4_semaphore) for item in all_items]
    
    proof_results = []
    successful_proofs = 0
    for f in tqdm(asyncio.as_completed(proof_tasks), total=len(proof_tasks), desc="Proving Theorems"):
        result = await f
...
@app.command()
def run(
    input_json: Path = typer.Argument(..., help="Path to Stage 07 reflowed sections JSON.", exists=True),
    output_dir: Path = typer.Option("data/results/pipeline", "-o", help="Parent directory for pipeline results."),
    skip_proving: bool = typer.Option(False, "--skip-proving", help="Only extract requirements without running the Lean 4 prover."),
):
    """
    ...
    try:
        if sampler and not gpu_metrics_available():
            diagnostics.append(make_event("08_lean4_theorem_prover","info","gpu_metrics_unavailable","NVML not available; GPU metrics disabled",{}))
    except Exception:
        pass
...
def debug_bundle(
    bundle: Path = typer.Argument(..., exists=True, file_okay=True, dir_okay=False, readable=True, help="Bundle with key 'reflowed_sections' (Stage 07 output-compatible)"),
    output_dir: Path = typer.Option("data/results/pipeline", "-o", help="Parent directory for pipeline results."),
    skip_proving: bool = typer.Option(True, "--skip-proving/--no-skip-proving", help="Skip Lean proving for debug runs."),
):
