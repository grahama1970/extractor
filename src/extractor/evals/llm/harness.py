...
async def chat_call(model: str, system_text: str, user_text: str, image_url: Optional[str], *, timeout: int = 60) -> CallResult:
    ...
    if not (isinstance(content, str) and content.strip()) and image_url:
        ...
        results2 = await litellm_call([params2], wrap_json=False, concurrency=1, desc="LLM Eval (no image)")
    ...
    return CallResult(content=content or '', parsed=parsed, usage=usage, provider_cost_reported=provider_cost, raw_object=None)


def calc_cost(model: str, usage: Dict[str, Any], provider_cost_reported: Optional[float], pricing: Dict[str, Dict[str, float]]) -> Optional[float]:
...
def build_manifest(dataset_registry: Path, models_file: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "env": {k: os.environ.get(k) for k in ["OPENAI_API_KEY", "GOOGLE_API_KEY", "MOONSHOT_API_KEY"] if os.environ.get(k)},
    }
