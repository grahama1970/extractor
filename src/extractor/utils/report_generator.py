...
"""
    ...
    if (
        "annotation_learner" in pipeline_data["results"]
        and pipeline_data["results"]["annotation_learner"]["success"]
    ):
...
def _add_extraction_details(pipeline_data: Dict[str, Any]) -> str:
    ...
    if hierarchy_result.get("success"):
        for log_entry in pipeline_data["execution_log"]:
            if (
                log_entry["step"] == "Hierarchy Builder"
                and "Built hierarchy" in log_entry["details"]
            ):
...
def _generate_recommendations(pipeline_data: Dict[str, Any]) -> List[str]:
    ...
    if not failed_processors and not any("CRITICAL" in rec for rec in recommendations):
        recommendations.append(
            "SUCCESS: All processors completed successfully. Pipeline is working correctly."
        )
...
async def working_usage():
    ...
    sample_summary = {
        ...
        "pdf_path": "test.pdf",
        "total_duration": 10.5,
        "processors": {"total": 2, "successful": 2, "failed": 0},
        "results": {
