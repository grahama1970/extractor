...
from extractor.core.util import matrix_intersection_area, sort_text_lines
...
class LineBuilder(BaseBuilder):
    ...
    span_inline_math_overlap_threshold: Annotated[
        float, "The minimum overlap of a span with an inline math box to consider for removal"
    ] = 0.5
    char_inline_math_overlap_threshold: Annotated[
        float, "The minimum overlap of a character with an inline math box to consider for removal"
    ] = 0.5
    line_inline_math_overlap_threshold: Annotated[
        float, "The minimum overlap of a line with an inline math box to consider as a match"
    ] = 0.0
    line_inline_min_overlap_pct: Annotated[
        float, "The percentage of a provider line that has to be covered by a math line."
    ] = 0.3
    line_text_overlap_threshold: Annotated[
        float, "The minimum overlap of an equation with a text line to consider as a match"
    ] = 0.5
    ...
    use_llm: Annotated[bool, "Whether to use the LLM model for advanced processing."] = False
    ...
    def __init__(
        ...
        self.inline_detection_model = inline_detection_model  # Can be None if not available
    ...
    def __call__(self, document: Document, provider: PdfProvider):
        ...
        # Or if inline detection model is not available
        do_inline_math_detection = all(
            [
                self.inline_detection_model is not None,
                len(document.contained_blocks([BlockTypes.Equation, BlockTypes.TextInlineMath]))
                > 0,
    ...
    def get_all_lines(
        self, document: Document, provider: PdfProvider, do_inline_math_detection: bool
    ):
        ...
        detection_results, inline_detection_results = self.get_detection_results(
            page_images, run_detection, do_inline_math_detection
        )

        assert (
            len(detection_results)
            == len(inline_detection_results)
            == len(layout_good)
            == len(document.pages)
        )
        for document_page, detection_result, inline_detection_result, provider_lines_good in zip(
            document.pages, detection_results, inline_detection_results, layout_good
        ):
            provider_lines: List[ProviderOutput] = provider.page_lines.get(
                document_page.page_id, []
            )
            ...
            merged_detection_boxes = self.determine_math_lines(
                text_result=detection_result, inline_result=inline_detection_result
            )
            # Sort the lines to ensure that the order is preserved
            merged_detection_boxes = sort_text_lines(merged_detection_boxes)

            math_detection_boxes = [
                (i, box) for i, box in enumerate(merged_detection_boxes) if box.math
            ]
            nonmath_detection_boxes = [
                (i, box) for i, box in enumerate(merged_detection_boxes) if not box.math
            ]
            ...
            if provider_lines_good:
                ...
                page_lines[document_page.page_id].extend(
                    self.merge_provider_lines_inline_math(
                        provider_lines, merged_detection_boxes, image_size, page_size
                    )
                )
            else:
    ...
    def determine_math_lines(
        ...
        text_boxes = [TextBox(polygon=box.polygon) for box in text_result.bboxes]
    ...
    def merge_provider_lines_inline_math(
        self, provider_lines: List[ProviderOutput], text_lines: List[TextBox], image_size, page_size
    ):
        ...
        horizontal_provider_lines = [
            ...
            if provider_line.line.polygon.height
            < provider_line.line.polygon.width
            * 5  # Multiply to account for small blocks inside equations, but filter out big vertical lines
        ]
        provider_line_boxes = [p.line.polygon.bbox for _, p in horizontal_provider_lines]
        math_line_boxes = [
            PolygonBox(polygon=m.polygon).rescale(image_size, page_size).bbox for m in text_lines
        ]
            ...
            for ml in merge_lines[line_idx]:
                ...
                if prev_line:
                    close = (
                        ...
                        < self.inline_math_line_vertical_merge_threshold
        ...
        out_provider_lines = [
            (i, p) for i, p in enumerate(provider_lines) if i not in potential_merges
        ]
        for line_idx in filtered_merge_lines:
    ...
    def find_overlapping_math_chars(self, provider_line, math_line_polygon, remove_chars=False):
        ...
        if provider_line.chars is None:
            for span in spans:
                if (
                    span.polygon.intersection_pct(math_line_polygon)
                    > self.span_inline_math_overlap_threshold
                ):
                    math_overlaps = True
        ...
        assert len(spans) == len(
            provider_line.chars
        ), "Number of spans and characters in provider line do not match"
        for span, span_chars in zip(spans, provider_line.chars):
            if len(span_chars) == 0:
                continue

            char_intersections_areas = matrix_intersection_area(
                [char.polygon.bbox for char in span_chars], [math_line_polygon.bbox]
            ).max(axis=-1)
            char_intersections = char_intersections_areas / np.array(
                [char.polygon.area for char in span_chars]
            )
