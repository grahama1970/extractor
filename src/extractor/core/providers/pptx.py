...
class PPTXProvider:
    ...
    def _get_ai_slide_description(self, slide: Slide, slide_idx: int, slide_title: str) -> str:
        ...
        try:
            ...
            if visual_elements:
                ...
                content_desc = [f"{c} {t}" if c == 1 else f"{c} {t}s"
                                for t, c in element_counts.items()]
                if content_desc:
    ...
    def _extract_shape(self, shape: Any, slide_idx: int) -> List[BaseBlock]:
    ...
    def _extract_text(self, shape: Any, slide_idx: int) -> List[BaseBlock]:
        ...
        if shape.has_text_frame:
            ...
            if (len(all_paragraphs) > 1 and shape.is_placeholder and
                ...
                if list_items:
                    blocks.append(self._create_list_block(list_items, slide_idx))
            else:
                for _, text in all_paragraphs:
                    blocks.append(BaseBlock(
                        ...
                        metadata=BlockMetadata(
                            ...
                            language=None,
                            attributes={'slide_number': slide_idx + 1, 'shape_type': 'text'},
                            confidence=0.95
                        )
                    ))
        return blocks

    def _create_list_block(self, items: List[Dict[str, Any]], slide_idx: int) -> BaseBlock:
    ...
    def _extract_table(self, shape: Any, slide_idx: int) -> Optional[TableBlock]:
    ...
    def _extract_image(self, shape: Any, slide_idx: int) -> Optional[ImageBlock]:
        ...
        try:
            ...
            if len(image_bytes) > max_size:
                logger.warning(f"Image too large ({len(image_bytes)} bytes), skipping base64 encoding")
            ...
            alt_text = shape.name or f"Image from slide {slide_idx + 1}"
            if ai_description:
    ...
    def _extract_chart(self, shape: Any, slide_idx: int) -> Optional[BaseBlock]:
        ...
        try:
            ...
            try:
                chart_type_str = str(chart.chart_type) if hasattr(chart, 'chart_type') else 'Unknown'
            ...
            try:
                title = (
                    ...
                    if (hasattr(chart, 'has_title') and chart.has_title and hasattr(chart, 'chart_title'))
                    else 'Untitled'
                )
    ...
    def _extract_notes(self, notes_slide: Any, slide_idx: int) -> List[BaseBlock]:
