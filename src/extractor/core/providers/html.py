...
class HTMLProvider:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.use_trafilatura = self.config.get("use_trafilatura", True)
        ...
        for idx, li in enumerate(element.find_all('li', recursive=False)):
            ...
            if li.find(['table', 'ul', 'ol']):
                ...
                try:
                    ...
                    logger.warning(f"Markdownify failed for list item: {e}, falling back to plain text")
                    item_text = li.get_text(strip=True)
    ...
    def _blocks_from_trafilatura(self, json_str: str, filepath: Path) -> UnifiedDocument:
        ...
        for line in lines:
            if line.startswith("#"):
                ...
                block = BaseBlock(
                    ...
                    parent_id=None,
                    metadata=BlockMetadata(
                        attributes={"level": level}, confidence=1.0
                    ),
                )
                blocks.append(block)
                self.current_headers[level] = text
        ...
        return UnifiedDocument(
            ...
            hierarchy=hierarchy,
            metadata=metadata,
            full_text="\n".join(b.content for b in blocks if isinstance(b.content, str) and b.content.strip()),
    ...
    def _parse_dimension(self, value: Optional[str]) -> Optional[int]:
        ...
        try:
            ...
            if isinstance(value, str):
                ...
                match = re.match(r"(\d+(?:\.\d+)?)(px|%|em|rem|pt|pc|in|cm|mm|ex|ch|vw|vh|vmin|vmax)?", value.strip(), re.I)
                if match:
