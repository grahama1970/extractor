...
class XMLProvider:
    ...
    def extract_document(self, filepath: Union[str, Path]) -> UnifiedDocument:
        ...
        try:
            ...
            # Handle XML syntax errors for both lxml and ElementTree
            if "XMLSyntaxError" in str(type(e)) or "syntax" in str(e).lower():
                logger.error(f"XML syntax error in {filepath}: {e}")
                # Fallback: wrap multiple top-level nodes with a root element and retry
                try:
                ...
                logger.error(f"Permission denied reading XML file: {filepath}")
                raise PermissionError(f"Permission denied: {filepath}")
            else:
                logger.error(f"Unexpected error parsing XML {filepath}: {e}")
                # Try to provide a more helpful error message
                if "encoding" in str(e).lower():
                    raise ValueError(f"XML encoding error: {e}")
                elif "memory" in str(e).lower():
                    raise MemoryError(f"XML file too large to parse: {e}")
                else:
            ...
            # Try to provide a more helpful error message
            if "encoding" in str(e).lower():
                raise ValueError(f"XML encoding error: {e}") from e
            elif "memory" in str(e).lower():
                raise MemoryError(f"XML file too large to parse: {e}") from e
            else:
        ...
        for child in element:
            ...
            if child.tail and child.tail.strip():
                tail_block = BaseBlock(
                    ...
                    metadata=BlockMetadata(
                        attributes={"xml_path": current_path, "context": "tail_text"},
                        confidence=0.9,
                    ),
                )
    ...
    def _extract_table_from_element(
        self, table_elem, row_tags: List[str], cell_tags: List[str]
    ) -> Optional[TableBlock]:
        ...
        for row_idx, row in enumerate(rows):
            ...
            for col_idx, cell_elem in enumerate(row):
                ...
                table_cells.append(TableCell(row=row_idx, col=col_idx, content=content))
    ...
    def _create_table_from_repeated_structure(self, parent, elements: List) -> Optional[TableBlock]:
        ...
        for col_idx, col_name in enumerate(columns):
            cells.append(TableCell(row=0, col=col_idx, content=col_name))

        # Create data rows
        for row_idx, elem in enumerate(elements, 1):
            for col_idx, child in enumerate(elem):
                content = self._get_element_text(child)
                cells.append(TableCell(row=row_idx, col=col_idx, content=content))
        ...
        return TableBlock(
            ...
            metadata=BlockMetadata(
                attributes={"source_tag": parent.tag, "detection_method": "repeated_structure"},
                confidence=0.85,
            ),
        )
