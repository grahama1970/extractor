...
def extract_parameters(parameters_node: Node, language: str) -> List[Dict[str, Any]]:
    ...
    for child in parameters_node.children:
        ...
        param_info = {"name": None, "type": None, "default": None, "required": True}
        ...
        try:
            ...
            try:
                ...
                if isinstance(capture_results, dict):
                    ...
                    function_nodes = capture_results.get("function", []) + capture_results.get(
                        "method", []
                    )
                    for node in function_nodes:
                        ...
                        if "params" in capture_results:
                            for params_node in capture_results["params"]:
                                if (
                                    params_node.start_byte >= node.start_byte
                                    and params_node.end_byte <= node.end_byte
                                ):
                                    func_info["parameters"] = extract_parameters(
                                        params_node, language_id
                                    )
                                    break
                        ...
                        if "body" in capture_results:
                            for body_node in capture_results["body"]:
                                if (
                                    body_node.start_byte >= node.start_byte
                                    and body_node.end_byte <= node.end_byte
                                ):
                                    func_info["docstring"] = extract_docstring(
                                        body_node, language_id
                                    )
                        ...
                        if "class_body" in capture_results:
                            for body_node in capture_results["class_body"]:
                                if (
                                    ...
                                    class_info["docstring"] = extract_docstring(
                                        body_node, language_id
                                    )
                                    break
                ...
                # Handle list of tuples case (original expected format)
                    ...
                    for capture in capture_results:
                        ...
                        if name == "function" or name == "method":
                            ...
                            if current_function:
                                current_function["parameters"] = extract_parameters(
                                    node, language_id
                                )
                            ...
                            if current_function:
                                ...
                                current_function["return_type"] = extract_return_type(
                                    current_function.get("node", node)
                                )
                ...
                logger.warning(
                    f"Query-based extraction processing failed: {e}, falling back to traversal"
                )
...
def traverse_fallback(
    root_node: Node, code: str, language_id: str
    ...
    def traverse(node: Node):
        ...
        # Extract class definitions
            ...
            name_node = find_child_by_type(node, ("identifier", "type_identifier", "constant"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--language", "-l", type=str, help="Language identifier (if not using file)"
    )
    ...
    if args.file:
        ...
        if not metadata["tree_sitter_success"]:
            console.print(
                f"[red]Failed to extract metadata:[/red] {metadata.get('error', 'Unknown error')}"
            )
            exit(1)
        ...
        if metadata["classes"]:
            ...
            for cls in metadata["classes"]:
                class_table.add_row(
                    ...
                    (cls["docstring"] or "")[:50]
                    + ("..." if cls["docstring"] and len(cls["docstring"]) > 50 else ""),
                )
    ...
    # Display usage if no arguments provided
        ...
        console.print(
            "[yellow]Please provide either a file path or a language and code snippet.[/yellow]"
        )
