...
Module: claude_table_adapter.py
...
from extractor.core.services.claude_unified import unified_claude_service
...
class ClaudeTableMergeAdapter:
    ...
    async def _process_task(self, task_id: str, config: Dict[str, Any]):
        ...
        try:
            ...
            result = await self.claude.analyze_tables(
                table1_data=table1_data, table2_data=table2_data, context=context
            )
    ...
    def get_task_status(self, task_id: str) -> Dict[str, Any]:
        ...
        conn = sqlite3.connect(str(self.db_path))
        cursor = conn.execute(
            "SELECT status, result, created_at, updated_at FROM tasks WHERE task_id = ?", (task_id,)
        )
    ...
    def wait_for_completion(self, task_id: str, timeout: float = 300) -> Dict[str, Any]:
        ...
        try:
            return loop.run_until_complete(self._wait_for_completion_async(task_id, timeout))
        finally:
            loop.close()
    ...
    def get_all_tasks(self, status: Optional[TaskStatus] = None) -> List[Dict[str, Any]]:
        ...
        for row in cursor:
            tasks.append(
                {"task_id": row[0], "status": row[1], "created_at": row[2], "updated_at": row[3]}
            )
    ...
    def cleanup_old_tasks(self, days: int = 7):
        ...
        conn = sqlite3.connect(str(self.db_path))
        conn.execute("DELETE FROM tasks WHERE created_at < datetime('now', ?)", (f"-{days} days",))
