...
def convert_for_qa(
    pdf_path: Path, output_dir: Path, output_format: OutputFormat = OutputFormat.JSON
) -> Path:
    """
...
def display_validation_results(metadata: dict):
    ...
    if "corpus_validation" in metadata:
        ...
        table.add_row(
            "Corpus Validation", f" Performed (threshold: {validation.get('threshold', 'N/A')}%)"
        )
        table.add_row("Raw Corpus Length", f"{validation.get('raw_corpus_length', 0):,} characters")
    ...
    if "validation_issues" in metadata:
        ...
        table.add_row("Validation Issues", f"{len(issues)} found")
...
def convert(
    ...
    output_dir: Path = typer.Option(
        Path("./qa_output"), "--output-dir", "-o", help="Output directory for converted files"
    ),
...
def batch(
    input_dir: Path = typer.Argument(
        ..., help="Directory containing PDF files", exists=True, file_okay=False, dir_okay=True
    ),
    output_dir: Path = typer.Option(
        Path("./qa_output"), "--output-dir", "-o", help="Output directory for converted files"
    ),
...
def validate(
    json_path: Path = typer.Argument(
        ..., help="Path to converted JSON file", exists=True, file_okay=True, dir_okay=False
    )
):
    """
