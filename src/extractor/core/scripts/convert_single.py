...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Extract font metrics for first span in each block",
)
        ...
        if kwargs.get("extract_fonts", False):
            ...
            if hasattr(result, "metadata"):
                if not hasattr(result.metadata, "update"):
                    ...
                    meta_dict["font_extraction_note"] = (
                        "Font data not available in marker JSON output - only in internal Document object"
                    )
