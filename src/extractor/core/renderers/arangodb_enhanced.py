...
class ArangoDBEnhancedRenderer(BaseRenderer):
    ...
    def _extract_graph_structure(self, document: Document) -> Dict[str, Any]:
        ...
        vertices = {"documents": [], "sections": [], "blocks": [], "entities": [], "tables": []}
        ...
        doc_id = self._generate_id(
            "doc", document.filepath if hasattr(document, "filepath") else "unknown"
        )
        doc_vertex = {
            "_key": doc_id,
            ...
            for doc in docs:
                ...
                if not is_valid:
                    validation_errors.extend(
                        [f"{collection}/{doc.get('_key', 'unknown')}: {e}" for e in errors]
                    )
    ...
    def _extract_entities_from_block(
        self, block: Block, block_id: str, vertices: Dict, edges: Dict
    ):
    ...
    def _process_table(self, table_block: Block, block_id: str, vertices: Dict, edges: Dict):
        ...
        table_vertex = {
            ...
            "columns": (
                len(getattr(table_block, "cells", [[]])[0]) if hasattr(table_block, "cells") else 0
            ),
...
if __name__ == "__main__":
    ...
    doc = Document(
        ...
        pages=[
            PageGroup(
                blocks=[
                    SectionHeader(text="Introduction", level=1, bbox=[0, 0, 100, 20]),
                    TextBlock(
                        text="This document discusses John Smith from New York University.",
                        bbox=[0, 30, 100, 50],
                    ),
                    SectionHeader(text="Data Analysis", level=1, bbox=[0, 60, 100, 80]),
                    Table(
                        cells=[
                            ["Name", "Age", "City"],
    ...
    # Test renderer
    renderer = ArangoDBEnhancedRenderer({"extract_entities": True, "extract_relationships": True})
