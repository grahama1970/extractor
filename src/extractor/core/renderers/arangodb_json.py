...
class BlockOutputArangoDB(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = (
        None  # Additional metadata (extraction details, quality metrics, etc.)
    )
...
class ArangoDBRenderer(BaseRenderer):
    ...
    def __call__(self, document: Document) -> ArangoDBOutput:
        ...
        doc_id = (
            ...
            or f"{os.path.basename(document.filepath).split('.')[0]}_{uuid.uuid4().hex[:8]}"
        )
        ...
        for page in document.pages:
            ...
            raw_corpus_pages.append(
                PageCorpus(page_num=page.page_id, text=page_text, tables=page_tables)
            )
        ...
        return ArangoDBOutput(
            document=DocumentOutput(id=doc_id, pages=pages),
            metadata=metadata,
            validation=ValidationInfo(corpus_validation=corpus_validation),
            raw_corpus=RawCorpus(
                full_text=full_text, pages=raw_corpus_pages, total_pages=len(document.pages)
            ),
        )
    ...
    def _extract_page_blocks(self, document: Document, page) -> List[BlockOutputArangoDB]:
        ...
        page_blocks = [
            block for block in document.contained_blocks() if block.page_id == page.page_id
        ]
        ...
        for block in page_blocks:
            ...
            if block_type == "section_header":
                ...
                block_output = BlockOutputArangoDB(type=block_type, text=text, level=level)
                ...
                block_output = BlockOutputArangoDB(type=block_type, text=text, language=language)
                if breadcrumbs:
                    block_output.breadcrumbs = breadcrumbs
                blocks.append(block_output)
                ...
                block_output = BlockOutputArangoDB(
                    type=block_type, text=text, csv=csv_data, json_data=json_data
                )
                ...
                block_output = BlockOutputArangoDB(type=block_type, text=text)
                if breadcrumbs:
                    block_output.breadcrumbs = breadcrumbs
                blocks.append(block_output)
    ...
    def _extract_breadcrumbs_for_block(
        self, document: Document, block
                        ...
                        if hasattr(block, "polygon") and block.polygon:
                            ...
                            if block_y >= section_y:
                                if (
                                    ...
                                    or section_y > current_section.get("bbox", [0, 0, 0, 0])[1]
                                ):
    ...
    def _extract_page_tables(self, document: Document, page) -> List[Dict[str, Any]]:
        ...
        table_blocks = [
            block
            for block in document.contained_blocks([BlockTypes.Table])
            if block.page_id == page.page_id
        ]

        for table_block in table_blocks:
            table_data = {"text": table_block.raw_text(document).strip()}
    ...
    def _extract_document_metadata(
        self, document: Document, document_output: BlockOutputArangoDB
    ) -> Dict[str, Any]:
