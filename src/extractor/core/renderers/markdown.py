...
class SectionBreadcrumb:
    ...
    def get_breadcrumb_path(self) -> List[Dict[str, str]]:
        ...
        for level in sorted(self.hierarchy.keys()):
            ...
            path.append({"level": level, "title": title, "hash": section_hash})
        return path
...
class Markdownify(MarkdownConverter):
    ...
    def convert_math(self, el, text, convert_as_inline):
        ...
        if block:
            ...
            return (
                " " + self.inline_math_delimiters[0] + text + self.inline_math_delimiters[1] + " "
            )
    ...
    def _convert_heading(self, el, text, level):
        ...
        section_hash = el.get(
            "data-section-hash", hashlib.sha256(text.encode("utf-8")).hexdigest()[:16]
        )
...
class MarkdownRenderer(HTMLRenderer):
    ...
    include_breadcrumbs: Annotated[
        bool, "Whether to include section breadcrumbs in the output."
    ] = True
