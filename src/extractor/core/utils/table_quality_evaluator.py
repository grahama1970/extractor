...
Module: table_quality_evaluator.py
...
class TableQualityEvaluator:
    ...
    def __init__(self, config=None):
        ...
        self.quality_weights = self.config.get(
            "quality_weights",
            {"accuracy": 0.4, "completeness": 0.3, "consistency": 0.1, "whitespace": 0.2},
        )
    ...
    def calculate_table_quality(self, tables: List[Any]) -> Dict[str, Any]:
        ...
        for i, table in enumerate(tables):
            try:
                ...
                table_scores.append(
                    {"table_index": i, "metrics": quality_metrics, "weighted_score": weighted_score}
                )
    ...
    def find_best_table_extraction(
        ...
        for current_params in param_combinations:
            ...
            tables = self.extract_table_with_params(
                page_num, filepath, cache_enabled=cache_enabled, **params_with_bbox
            )
            if not tables:
            ...
            if current_quality > best_result.quality:
                best_result = ExtractionResult(
                    tables=tables, quality=current_quality, params=current_params
                )
                logger.info(
                    f"New best quality found: {current_quality:.4f} with params: {current_params}"
                )

            # Early exit if quality is good enough
            if current_quality >= 0.9:
                logger.info(
                    f"Found excellent quality table ({current_quality:.4f}), stopping search"
                )
                break

        logger.info(
            f"Best quality achieved: {best_result.quality:.4f} after {total_attempts} attempts"
        )
        return best_result.tables, best_result.params

    def extract_table_with_params(
        self, page_num: int, filepath: str, **params
    ) -> Optional[List[Any]]:
        """
        ...
        try:
            ...
            logger.error(
                f"Error extracting tables from page {page_num + 1} with params {params}: {str(e)}"
            )
            return None
        ...
        if page_to_test == -1:
            ...
            if "average_table_extraction_quality" not in quality_result:
                all_validation_failures.append(
                    "Missing 'average_table_extraction_quality' in quality result"
                )
            ...
            print(
                f"  - Average quality: {quality_result.get('average_table_extraction_quality', 0.0):.4f}"
            )
