...
DEFAULT_CONFIG = {
    ...
    "embedding": {
        ...
        ),  # Get from env or use 1024 as default
        "model": os.environ.get(
            "EMBEDDING_MODEL", "BAAI/bge-large-en-v1.5"
        ),  # Get from env or use BGE as default
    },
    "vector_search": {
...
def execute_aql_with_retry(
    db: StandardDatabase, query: str, bind_vars: Optional[Dict[str, Any]] = None
) -> Any:
    """
    ...
    try:
        ...
        if sys_db.has_database(db_name):
            ...
            sys_db.create_database(
                name=db_name, users=[{"username": username, "password": password, "active": True}]
            )
...
def ensure_collection(db: StandardDatabase, collection_name: str = "documents") -> bool:
    """
...
def check_vector_index_exists(db: StandardDatabase, collection_name: str, field_name: str) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            if count_result < nlists:
                logger.warning(
                    f"Not enough documents with embeddings to create vector index. Found {count_result}, need at least {nlists}."
                )
                logger.warning(
                    "Vector index creation may fail. Consider adding more documents or reducing nLists value."
                )
...
def validate_embedding_dimensions(
    db: StandardDatabase, collection_name: str, embedding_field: str, expected_dimension: int
    ...
    try:
        ...
        if type_errors:
            ...
            return (
                False,
                f"Found {len(type_errors)} documents with non-array embeddings (e.g., {error_keys})",
            )
        ...
        if dimension_errors:
            ...
            return (
                False,
                f"Found {len(dimension_errors)} documents with wrong embedding dimensions (expected {expected_dimension}): {error_examples}",
            )
...
def setup_arango_for_vector_search(
    ...
    vector_index_nlists = (
        vector_index_nlists or DEFAULT_CONFIG["vector_search"]["vector_index_nlists"]
    )
    ...
    if not vector_index_exists:
        logger.info(
            "Vector index does not exist yet - it will be created after documents are added"
        )

    logger.info(
        f" ArangoDB setup completed successfully. Steps completed: {', '.join(steps_completed)}"
    )
    return True
...
def main():
    ...
    try:
        ...
        if not ensure_collection(db, collection_name):
            validation_passed = False
            validation_failures["ensure_collection"] = (
        ...
        if not ensure_arangosearch_view(
            db, view_name, collection_name, DEFAULT_CONFIG["embedding"]["field"]
        ):
            validation_passed = False
            validation_failures["ensure_arangosearch_view"] = (
                f"Failed to ensure ArangoSearch view '{view_name}' exists"
            )
        ...
        if doc_count < DEFAULT_CONFIG["vector_search"]["vector_index_nlists"]:
            logger.warning(
                f"Collection has only {doc_count} documents. Need at least {DEFAULT_CONFIG['vector_search']['vector_index_nlists']} for vector index."
            )
            ...
            if not ensure_vector_index(
                ...
                validation_passed = False
                validation_failures["ensure_vector_index"] = (
                    f"Failed to ensure vector index for '{embedding_field}' exists"
                )
