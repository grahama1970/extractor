...
# Retry decorators
def retry_on_network_error(max_attempts: int = 3, wait_min: float = 4.0, wait_max: float = 10.0):
    """Retry decorator for network operations."""
    return retry(
        stop=stop_after_attempt(max_attempts),
        wait=wait_exponential(multiplier=1, min=wait_min, max=wait_max),
        retry=retry_if_exception_type((ConnectionError, DatabaseConnectionError, TimeoutError)),
...
def retry_on_llm_error(max_attempts: int = 3, wait_min: float = 2.0, wait_max: float = 60.0):
    """Retry decorator for LLM operations with backoff."""
    return retry(
        stop=stop_after_attempt(max_attempts),
        wait=wait_exponential(multiplier=2, min=wait_min, max=wait_max),
        retry=retry_if_exception_type((LLMProcessingError, TimeoutError)),
        before=log_retry_attempt,
        after=log_retry_success,
        reraise=True,
    )
...
class ErrorHandler:
    ...
    def __init__(self, operation: str, raise_on_error: bool = True, default_return: Any = None):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        logger.error(f"Operation failed: {self.operation} - " f"{exc_type.__name__}: {exc_val}")
        ...
        if isinstance(exc_val, PDFExtractionError):
            ...
            wrapped = PDFExtractionError(
                f"{self.operation} failed: {exc_val}", details=error_details
            )
            if self.raise_on_error:
...
class AsyncErrorHandler:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        # Log the error
        logger.error(
            f"Async operation failed: {self.operation} - " f"{exc_type.__name__}: {exc_val}"
        )
            ...
            for param_name, validator in validations.items():
                if param_name in bound.arguments:
                    ...
                    if not validator(value):
                        raise ValidationError(f"Invalid {param_name}: {value} failed validation")
