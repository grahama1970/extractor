...
def extract_sections_from_markdown(markdown_content: str) -> List[Dict[str, Any]]:
    ...
    for i, section in enumerate(section_data):
        ...
        end_pos = (
            section_positions[i + 1][0] if i < len(section_data) - 1 else len(markdown_content)
        )
...
def get_section_path(breadcrumb: Dict[str, Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    for level in sorted(breadcrumb.keys()):
        if isinstance(breadcrumb[level], dict):
            ...
            result.append({"title": title, "hash": section_hash, "level": int(level)})
        elif isinstance(breadcrumb[level], str):
            # Handle legacy format where only blockId is stored
            result.append({"title": f"Section {level}", "hash": "", "level": int(level)})
    return result
...
def extract_from_markdown(markdown_content: str) -> Dict[str, Any]:
    ...
    for section in sections:
        level = section["level"]
        section_hash = next(
            (item["hash"] for item in section["section_path"] if item["level"] == level), ""
        )
    ...
    return {"sections": sections, "section_hierarchy": section_hierarchy}
