...
Module: relationship_extractor.py
Description: Functions for relationship extractor operations
...
Example:
    ```python
    from extractor.core.utils.relationship_extractor import extract_relationships_from_marker
...
def extract_relationships_from_marker(marker_output: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for page_idx, page in enumerate(document.get("pages", [])):
        ...
        relationships.append({"from": doc_id, "to": page_id, "type": "CONTAINS"})
        ...
        for block_idx, block in enumerate(page.get("blocks", [])):
            ...
            relationships.append({"from": page_id, "to": block_id, "type": "CONTAINS"})
            ...
            if block_type == "section_header":
                ...
                section_map[block_id] = {"text": block_text, "level": section_level}
            else:
                # Store content block
                content_map[block_id] = {"text": block_text, "type": block_type}
    ...
    for section_id, section_info in section_map.items():
        ...
        sections_by_level[level].append({"id": section_id, "info": section_info})
    ...
    for content_id, content_info in content_map.items():
        ...
        for section_id, section_info in section_map.items():
            if page_id_map.get(section_id) == content_page:
                page_sections.append({"id": section_id, "info": section_info})
        ...
        if page_sections:
            ...
            relationships.append({"from": containing_section, "to": content_id, "type": "CONTAINS"})
...
def extract_section_tree(marker_output: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for section in sections:
        ...
        # Create node
        node = {"id": section["id"], "text": section["text"], "level": level, "children": []}
...
if __name__ == "__main__":
    ...
    test_doc = {
        "document": {
            ...
            "pages": [
                {
                    "blocks": [
                        {"type": "section_header", "text": "Introduction", "level": 1},
                        {"type": "text", "text": "This is introduction text."},
                        {"type": "section_header", "text": "Methods", "level": 1},
                        {"type": "text", "text": "Methods description."},
                        {"type": "section_header", "text": "Experiment Setup", "level": 2},
                        {"type": "text", "text": "Details of experiment setup."},
                    ]
                }
            ],
        }
    ...
    try:
        ...
        if len(relationships) != expected_count:
            ...
            for rel in relationships:
                if (
                    rel["type"] == "CONTAINS"
                    and "section" in rel["from"]
                    and "section" in rel["to"]
                ):
    ...
    test_doc_complex = {
        "document": {
            ...
            "pages": [
                {
                    "blocks": [
                        {"type": "section_header", "text": "Abstract", "level": 1},
                ...
                {
                    "blocks": [
                        {"type": "section_header", "text": "Background", "level": 2},
                        {"type": "text", "text": "Background information."},
                        {"type": "section_header", "text": "Methods", "level": 1},
                        {"type": "text", "text": "Methods description."},
                    ]
                },
    ...
    try:
        ...
        if len(complex_relationships) < min_expected:
            failure_msg = (
                f"Expected at least {min_expected} relationships, got {len(complex_relationships)}"
            )
            all_validation_failures.append(failure_msg)
            logger.error(failure_msg)
        else:
