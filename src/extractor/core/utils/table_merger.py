...
from extractor.core.utils.table_comparison import should_merge_tables
...
class TableMerger:
    ...
    def __init__(self, config=None):
        ...
        self.merge_thresholds = self.config.get(
            "merge_thresholds",
            {
    ...
    def check_tables_for_merges(
        self, tables: List[Dict[str, Any]], page_heights: Optional[Dict[int, float]] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _merge_tables(self, tables: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if "parsing_report" in tables[0]:
            merged_table["parsing_report"] = {
                ...
                "whitespace": sum(t.get("parsing_report", {}).get("whitespace", 0) for t in tables)
                / len(tables),
    ...
    def _merge_table_data(
        self, table_data_list: List[List[Dict[str, Any]]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def merge_document_tables(self, document: Document) -> None:
        ...
        for page in document.pages:
            ...
            for block in page.contained_blocks(document, (BlockTypes.Table,)):
                ...
                table_data = [
                    {str(col): "" for col in range(max_col + 1)} for _ in range(max_row + 1)
                ]
    ...
    def _apply_merges_to_document(
        self, document: Document, merged_tables: List[Dict[str, Any]]
    ) -> None:
        """
        ...
        for merged_table in merged_tables:
            ...
            # Update table metadata with merge information
            if hasattr(base_block, "merge_info"):
                base_block.merge_info = {
                    "merged": True,
                    "source_ids": [b.id for _, b in source_blocks],
                    "original_tables": len(source_blocks),
                }
...
if __name__ == "__main__":
        ...
        if merged_table:
            ...
            if len(merged_data) != 4:  # 2 rows from each table
                all_validation_failures.append(
                    f"Expected 4 rows in merged table, got {len(merged_data)}"
                )
    ...
    try:
        ...
        bboxes = [[100, 100, 300, 200], [150, 150, 350, 250], [50, 200, 250, 300]]
    ...
    try:
        ...
        if len(merged_data) != expected_len:
            all_validation_failures.append(
                f"Expected {expected_len} rows in merged data, got {len(merged_data)}"
            )
