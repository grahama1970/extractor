...
class DocumentDebugger:
    ...
    @staticmethod
    def setup_logging(
        log_file: str = "marker_debug.log", console_level: str = "INFO", file_level: str = "DEBUG"
    ):
    ...
    @staticmethod
    def log_section_hierarchy(
        document, debug_dir: str = "debug", msg: str = "Current section hierarchy:"
    ):
        """
    ...
    def breakpoint_after_section(
        ...
        for section in sections:
            title_match = (
                section_title and section_title.lower() in section.raw_text(document).lower()
            )
            level_match = (
                level is not None
                and hasattr(section, "heading_level")
                and section.heading_level == level
            )
        ...
        if not match_found:
            logging.warning(
                f"No section matching criteria found (title: {section_title}, level: {level})"
            )
            return None
