...
class FontHeaderProcessor(BaseProcessor):
            ...
            for block in page.children:
                    ...
                    if block_polygon:
                        for other_block in page.children:
                            if hasattr(other_block, "font") and hasattr(other_block, "font_size"):
                                ...
                                if (
                                    other_polygon
                                    and block_polygon.intersection_pct(other_polygon) > 0.5
                                ):
                ...
                if fonts and font_sizes:
                    ...
                    self._font_data_cache[block.id] = {
                        ...
                        "first_font_info": {"font": first_font, "size": first_size},
                    }

                    logger.debug(
                        f"Extracted font data for block {block.id}: {first_font} @ {first_size:.1f}pt (first font)"
                    )
    ...
    def _process_page(self, page, document: Document):
        ...
        for i, block in enumerate(blocks):
            ...
            confidence, reasons = self._analyze_font_transition(current_font, prev_font, next_font)
    ...
    def _analyze_font_transition(
        self, current_font: Dict, prev_font: Optional[Dict], next_font: Optional[Dict]
        ...
        if prev_font and self.check_both_neighbors:
            ...
            if current_base != prev_base:
                confidence += 0.3
                reasons.append(
                    f"font_differs_from_above: {prev_font['font']} → {current_font['font']}"
                )
            ...
            if size_diff >= self.min_font_size_difference:
                confidence += 0.2
                reasons.append(
                    f"size_differs_from_above: {prev_font['size']:.1f} → {current_font['size']:.1f}"
                )
        ...
        if next_font:
            ...
            if current_base != next_base:
                confidence += 0.3
                reasons.append(
                    f"font_differs_from_below: {current_font['font']} → {next_font['font']}"
                )
            ...
            if size_diff >= self.min_font_size_difference:
                confidence += 0.2
                reasons.append(
                    f"size_differs_from_below: {current_font['size']:.1f} → {next_font['size']:.1f}"
                )

            # Extra confidence for header→body font pattern
            if any(hf in current_base for hf in self.header_fonts) and any(
                bf in next_base for bf in self.body_fonts
            ):
                confidence += 0.3
        ...
        if self.check_both_neighbors and prev_font and next_font:
            if current_base != self._get_base_font(
                prev_font["font"]
            ) and current_base != self._get_base_font(next_font["font"]):
                confidence += 0.2
    ...
    def _correct_block_type(
        self, block: Block, confidence: float, reasons: List[str], document: Document
    ):
        ...
        logger.info(
            f"Correcting block type to SectionHeader: '{text}...' (confidence: {confidence:.2f})"
        )
