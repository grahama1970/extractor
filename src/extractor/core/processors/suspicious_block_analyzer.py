...
class SuspiciousBlockAnalyzer:
    ...
    def create_analysis_prompt(self, batch: List[Dict[str, Any]], annotations: List[Dict[str, Any]]) -> str:
...
"""
        ...
        for ann in annotations:
            if ann.get('type') and ann.get('content'):
                prompt += f"- {ann['type']}: \"{ann['content']}\" on page {ann.get('page', 'unknown')}\n"
...
"""
    ...
    def batch_suspicious_blocks(self, suspicious_blocks: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
    ...
    async def analyze_suspicious_blocks(
        ...
        for item in suspicious_blocks[:3]:  # Show first 3
            block = item['block']
            logger.info(f"Block {item['index']}: {block.get('type')} - \"{block.get('text', '')[:40]}...\"")
...
def main():
    ...
    for decision in decisions:
        logger.info(f"Block {decision['block_index']}: {decision['action']} → {decision['new_type']}")
