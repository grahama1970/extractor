...
class SuspiciousHeaderFixer(BaseProcessor):
    ...
    HEADER_WHITELIST: set[str] = {
        "Introduction", "Conclusion", "Discussion", "Results", "Methods",
        "Methodology", "Overview", "Background", "Abstract", "Summary",
        "Acknowledgements", "References", "Appendix", "Notes", "Notation",
        "Definitions", "Examples", "See also", "Related work", "Limitations",
        "Acknowledgments", "Materials and methods", "Experimental setup",
        "System overview", "Design", "Implementation", "Evaluation",
        "Future work", "Conclusions", "Discussion and conclusions",
    }
    ...
    def _demote_improper_headers_with_doc(self, blocks: List[Block], document: Document) -> int:
        ...
        for b in blocks:
            ...
            if text:
                ...
                if (text.endswith(',') or 
                    (text[0].islower() and not text.startswith('●')) or
                    text.startswith('As ') or
                    text.startswith('For ')):
        ...
        for i, block in enumerate(blocks):
            ...
            if block.block_type == BlockTypes.SectionHeader:
                ...
                if i > 0:
                    ...
                    if prev_block.block_type in {BlockTypes.SectionHeader, BlockTypes.Figure, BlockTypes.Table}:
                        is_suspicious = True
    ...
    def _is_confident_header(self, b: Block) -> bool:
        ...
        if any(fuzz.ratio(txt.lower(), w.lower()) >= self.WHITELIST_SIMILARITY for w in self.HEADER_WHITELIST):
            return True
    ...
    def _same_font(self, a: Block, b: Block, document: Document, fitz_doc=None) -> bool:
        ...
        def _first_span_font_from_doc(block: Block):
            try:
                ...
                if spans:
                    s0 = spans[0]
                    return {"name": getattr(s0, "font", None), "size": float(getattr(s0, "font_size", 0) or 0)}
            except Exception:
                pass
            return None
        ...
        def _first_span_font_from_pymupdf(block: Block):
            ...
            try:
                ...
                bbox = getattr(block, "polygon", None).bbox if getattr(block, "polygon", None) else None
                if not bbox:
                    return None
                x0, y0, x1, y1 = bbox
                def _overlap(b1, b2):
...
if __name__ == '__main__':
    ...
    class DebugBlock:
        def __init__(self, id: int, block_type: BlockTypes, text: str = "",
            ...
            self.validation_metadata = validation_metadata if validation_metadata is not None else {}
    ...
    def print_blocks_state(title: str, blocks: list[DebugBlock]):
        ...
        for i, b in enumerate(blocks):
            ...
            print(f"  {i+1:02d}. ID={b.id:<2} Type={b.block_type.value:<15} Text={text_preview:<50}{suspicious_flag}")
        print("=" * (22 + len(title)))
    ...
    def run_debug_test():
        ...
        mock_blocks = [
            ...
            DebugBlock(id=1, block_type=BlockTypes.SectionHeader, text="Introduction", polygon=MockPolygon(y0=50, y1=60)),
            # Case 2: A header that should be demoted (looks like a sentence).
            DebugBlock(id=2, block_type=BlockTypes.SectionHeader, text="This is not a real header.", polygon=MockPolygon(y0=80, y1=90)),
            # Case 3 & 4: A split header that needs merging.
            DebugBlock(id=3, block_type=BlockTypes.Text, text="2.1 A Split Header (", validation_metadata={"first_span_font": {"size": 14}}, polygon=MockPolygon(y0=100, y1=110)),
            DebugBlock(id=4, block_type=BlockTypes.Text, text="Continued)", validation_metadata={"first_span_font": {"size": 14}}, polygon=MockPolygon(y0=111, y1=121)),
            # Case 5: A confident ALL-CAPS header. Should not be changed.
            DebugBlock(id=5, block_type=BlockTypes.SectionHeader, text="METHODOLOGY", polygon=MockPolygon(y0=140, y1=150)),
            # Case 6: A table to provide context for the next block.
            DebugBlock(id=6, block_type=BlockTypes.Table, text="Table 1...", polygon=MockPolygon(y0=160, y1=200)),
            # Case 7: A header immediately following a table. Should be flagged as suspicious.
            DebugBlock(id=7, block_type=BlockTypes.SectionHeader, text="Possibly a table title", polygon=MockPolygon(y0=210, y1=220)),
            # Case 8 & 9: Two adjacent tables that should be merged.
            DebugBlock(id=8, block_type=BlockTypes.Table, text="Table part A.", polygon=MockPolygon(y0=240, y1=260)),
            DebugBlock(id=9, block_type=BlockTypes.Table, text="Table part B.", polygon=MockPolygon(y0=261.5, y1=280)), # 1.5pt gap
            # Case 10 (THE BUG): A valid table followed by a table with no polygon. Should NOT crash.
            DebugBlock(id=10, block_type=BlockTypes.Table, text="This table is fine.", polygon=MockPolygon(y0=300, y1=320)),
            DebugBlock(id=11, block_type=BlockTypes.Table, text="This one is missing geometry.", polygon=None),
        ]
