...
class BaseLLMProcessor(ABC):
    ...
    async def _call_llm(self, prompt: str, image: Any = None, block: Any = None,
                  response_schema: Any = None, max_retries: int = 3) -> Optional[Dict[str, Any]]:
        """
    ...
    def _find_blocks_in_region(self, blocks: List[Dict[str, Any]], bbox: List[float],
                              page: int = None, tolerance: float = 5.0) -> List[Dict[str, Any]]:
        """
        ...
        for block in blocks:
            ...
            if (bx1 + tolerance >= x0 and bx0 - tolerance <= x1 and
                by1 + tolerance >= y0 and by0 - tolerance <= y1):
