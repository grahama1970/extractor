...
class BaseLLMSimpleBlockProcessor(BaseLLMProcessor):
    ...
    def __call__(self, result, prompt_data, document): pass
...
class LLMSimpleBlockMetaProcessor(BaseLLMProcessor):
    ...
    """
    def __init__(self, processor_lst: List[BaseLLMSimpleBlockProcessor], llm_service: BaseService, config: Dict[str, Any] = None):
            ...
            for future in pending:
                try:
                    ...
                    if not result or len(str(result)) < self.min_suspicious_length:
                        ...
                        block['metadata'][MetadataKey.SUSPICIOUS_REASON] = "LLM returned an empty or very short response."
    ...
    def get_response(self, prompt_data: Dict[str, Any]):
        return self.llm_service(prompt_data["prompt"], prompt_data.get("image"), prompt_data["block"], prompt_data.get("schema"))
...
async def working_usage():
    ...
    class MockSimpleProcessor(BaseLLMSimpleBlockProcessor):
        def inference_blocks(self, document): return [{'block_type': 'Text', 'text': 'Block 1'}, {'block_type': 'Text', 'text': 'Block 2'}]
        def block_prompts(self, document): return [{'prompt': 'A good prompt', 'block': {}}, {'prompt': 'A prompt to fail', 'block': {}}]
