...
class SuspiciousBlockProcessor(BaseProcessor):
    ...
    def __init__(self):
        ...
        self.suspicious_patterns = [
            ...
            (r'^\d+\.\d+\.?\d*\.?\s+\w+.*\($', 'incomplete_header'),  # "4.1.5.4. BHT (Branch History"
            ...
            for i, block in enumerate(blocks):
                ...
                if i > 0:
                    ...
                    if hasattr(prev_block, 'text') and prev_block.text:
                        ...
                        if (prev_block.text.rstrip().endswith('(') and 
                            not text.startswith('(')):
                            issues.append('possible_split_parentheses')
                        
                        # Check for continuation
                        if (not prev_block.text.rstrip().endswith('.') and
                            text[0].islower()):
                ...
                if i < len(blocks) - 1:
                    ...
                    if hasattr(next_block, 'text') and next_block.text:
                        ...
                        if (text.rstrip().endswith('(') and 
                            next_block.text.strip().endswith(')')):
                ...
                if issues:
                    ...
                    if not hasattr(block, 'validation_metadata') or block.validation_metadata is None:
                    ...
                    logger.debug(f"Marked block {i} as suspicious: {issues[:2]}... Text: '{text[:40]}...'")
