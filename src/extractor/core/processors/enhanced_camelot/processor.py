...
Module: processor.py
Description: Implementation of processor functionality
...
if __name__ == "__main__":
    ...
    try:
        ...
        mock_detection_model = Magicobject()
        mock_recognition_model = Magicobject()
        mock_table_rec_model = Magicobject()
    ...
    try:
        if CAMELOT_AVAILABLE:
            ...
            processor.quality_evaluator.find_best_table_extraction = MagicMock(return_value=(
                [MagicMock(df=MagicMock(empty=False))],  # Mock tables
                {"flavor": "lattice", "line_scale": 15}   # Mock parameters
            ))
            
            # Mock calculate_table_quality
            processor.quality_evaluator.calculate_table_quality = MagicMock(return_value={
