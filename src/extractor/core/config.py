...
class ExtractionConfig(BaseSettings):
    ...
    output_dir: Path = Field(default=Path("/tmp"), description="Output directory for temporary files")
    ...
    # Resource limits
    max_file_size_mb: int = Field(default=100, ge=1, le=1000, description="Maximum PDF file size in MB")
    max_pages: int = Field(default=1000, ge=1, le=10000, description="Maximum number of pages to process")
    max_annotations: int = Field(default=10000, ge=1, le=100000, description="Maximum annotations per PDF")
    processing_timeout_sec: int = Field(default=300, ge=10, le=3600, description="Processing timeout in seconds")
    max_embedding_batch: int = Field(default=100, ge=1, le=1000, description="Maximum batch size for embeddings")
    
    # Performance settings
    parallel_workers: int = Field(default=4, ge=1, le=16, description="Number of parallel workers")
    batch_size: int = Field(default=10, ge=1, le=100, description="Default batch size for processing")
    
    # Extraction thresholds
    header_max_length: int = Field(default=15, ge=5, le=50, description="Maximum header length in words")
    context_distance_mm: float = Field(default=100.0, ge=10.0, le=500.0, description="Context distance in mm")
    page_break_threshold: float = Field(default=0.85, ge=0.0, le=1.0, description="Page break confidence threshold")
    ...
    redis_max_connections: int = Field(default=50, ge=10, le=1000, description="Redis connection pool size")
    ...
    llm_max_tokens: int = Field(default=4096, ge=100, le=32000, description="Maximum tokens for LLM")
