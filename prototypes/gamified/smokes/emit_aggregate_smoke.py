...
def main():
    ...
    cmd = [sys.executable, '-m', 'prototypes.gamified.cli', 'run', '--codebase', '.', '--prompt-file', PROMPT, '--instances', '2', '--emit-only', '--no-autostart-backend', '--no-start-dashboard']
    p = sh(cmd)
    if p.returncode != 0:
    ...
    # Aggregate
    p2 = sh([sys.executable, '-m', 'prototypes.gamified.cli', 'run', '--codebase', '.', '--run-id', rid, '--aggregate-only', '--no-autostart-backend', '--no-start-dashboard'])
    if p2.returncode != 0:
