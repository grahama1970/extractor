...
def _show_diff(raw: str, compiled: str) -> str:
    diff = difflib.unified_diff(raw.splitlines(), compiled.splitlines(), fromfile="research", tofile="compiled", lineterm="")
    return "\n".join(diff)
def _call_llm_compile(research_text: str) -> str:
    ...
    if not text:
        ...
        if fallback_model != model:
            try:
                resp2 = litellm.completion(model=fallback_model, messages=messages, temperature=0.2, max_tokens=2500)
...
def compile(
    ...
    out_path: Optional[Path] = typer.Option(None, "--out", "-o", help="Write compiled prompt to this path"),
    rules_path: Path = typer.Option(Path("prototypes/gamified/rules/prompt_optimization.yaml"), "--rules", help="Rules file for POP validation"),
