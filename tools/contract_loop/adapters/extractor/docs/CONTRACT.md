...
- Each step writes to `data/results/<run>/<step>/{json_output,visual_output,logs}`.
...
- Use `tools/contract_loop/verify_pipeline_contract.py` to run each step, verify artifacts, and retry per-step up to `--max-tries` (wrapper: `scripts/verify_pipeline_contract.py`).
- Deterministic mode skips LLM steps; full mode runs all steps:
  - `python tools/contract_loop/verify_pipeline_contract.py --pdf <PDF> --mode deterministic`
  - `python tools/contract_loop/verify_pipeline_contract.py --pdf <PDF> --mode full --min-requirements 1`
- Fixture-specific expectations are stored under `tools/contract_loop/adapters/extractor/fixtures/` and can be enforced with:
  - `python tools/contract_loop/verify_pipeline_contract.py --pdf <PDF> --fixture tools/contract_loop/adapters/extractor/fixtures/<fixture>.json --mode deterministic`
- Optional LLM reasonableness judging (Codex exec) can be enabled with:
  - `python tools/contract_loop/verify_pipeline_contract.py --pdf <PDF> --fixture tools/contract_loop/adapters/extractor/fixtures/<fixture>.json --mode full --llm-judge`
- Lean4 verification can be skipped explicitly (default behavior until upstream fixes land):
  - `python tools/contract_loop/verify_pipeline_contract.py --pdf <PDF> --mode full --skip-lean4`
- Fixture files may include `pdf_sha256` and `contract_version`; the verifier enforces the hash when present.
...
- Deterministic CI (offline):
  - Preferred: `python tools/contract_loop/verify_pipeline_contract.py --pdf <PDF> --mode deterministic`
  - No network or SciLLM calls.
- Live CI (self-hosted):
  - Run LLM-inclusive pipeline + `python tools/contract_loop/verify_pipeline_contract.py --pdf <PDF> --mode full --min-requirements 1`.
