...
def analyze_table_failure(fixture_dir: Path, expected: Dict, actual: Dict) -> List[str]:
    ...
    results_dir = fixture_dir.parent.parent.parent / "data" / "results" / "pipeline" / "05_table_extractor" / "json_output" / "05_tables.json"
    actual_tables = []
    ...
    if cnt_exp > cnt_act:
        proposals.append(f"[Table] Expected {cnt_exp} tables, found {cnt_act}. Try increasing line_scale (Config Key: camelot.line_scale -> 20)")
    elif cnt_act > cnt_exp:
        proposals.append(f"[Table] Found {cnt_act} tables, expected {cnt_exp}. Try increasing text_heavy_threshold (Config Key: heuristics.avg_words -> 6.0)")
...
def analyze_requirement_failure(fixture_dir: Path, expected: Dict, actual: Dict) -> List[str]:
    ...
    results_dir = fixture_dir.parent.parent.parent / "data" / "results" / "pipeline" / "08_extract_requirements" / "json_output" / "08_requirements.json"
    actual_reqs = []
    actual_ids = set()
    if results_dir.exists():
    ...
    for req in exp_reqs:
        ...
        if rid and rid not in actual_ids:
            ...
            proposals.append(f"[Req] Missed ID {rid}. Suggest adding prefix '{prefix}' to domain_patterns.yml id_prefixes")
