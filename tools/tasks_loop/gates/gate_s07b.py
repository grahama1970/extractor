...
def checks(fixture_name: str):
    ...
    if not db_path.exists():
        raise GateError("Database not found", expected="pipeline.duckdb", actual="missing", file=str(db_path))

    con = duckdb.connect(str(db_path), read_only=True)
    try:
        count = con.execute("SELECT count(*) FROM merged_content").fetchone()[0]
        if count == 0:
             raise GateError("merged_content table is empty", expected=">0 rows", actual=0, file=str(db_path))
