...
def checks(fixture_name: str):
    ...
    missing_enrichment = [
        t.get("id") for t in tables if not t.get("llm_title") or not t.get("llm_description")
    ]

    if missing_enrichment:
        print(
            f"⚠️ WARNING: {len(missing_enrichment)} tables missing enrichment (llm_title/llm_desc)"
        )
