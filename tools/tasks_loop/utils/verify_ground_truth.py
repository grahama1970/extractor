...
def load_actual_from_duckdb(db_path: Path) -> List[Dict[str, Any]]:
    ...
    for rid, text, rtype in reqs:
        actual.append({"id": rid, "text": text, "type": rtype})

    # Get Tables (Standard Pipeline Table)
    # Tables are often in 'tables' table.
    try:
        tables = con.execute(
            "SELECT id, llm_title, llm_description, csv_data FROM tables"
        ).fetchall()
    ...
    for eid, exp in expected_map.items():
        ...
        if eid in actual_map:
            ...
            if best_match and best_score >= threshold:
                ...
                if not json_output:
                    print(
                        f"   [MISS] {eid}: Best Score {best_score:.2f}% (Threshold {threshold}) using partial_ratio"
                    )
                misses.append(eid)
                continue
                ...
                matches_fail.append(
                    {
                        "id": eid,
                        "score": sim,
                        "reason": f"STRICT_FAIL (Sim: {sim:.2f}%) [{match_type}]",
                    }
                )
            continue
