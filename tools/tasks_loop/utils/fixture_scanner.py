...
class PDFScanner:
    ...
    def _is_header(self, span: dict) -> bool:
        ...
        if is_numbered and self.verbose:
            print(
                f"[DEBUG] Rejected Numbered: '{text}' Size:{span['size']} Body:{self.body_size} Bold:{is_bold} Caps:{is_caps}"
            )
    ...
    def detect_tables(self, page_num: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for idx, tbl in enumerate(tables):
                ...
                self._log(
                    f"Page {page_num+1}: Detected table {idx+1} ({num_rows}x{num_cols}) at {bbox}"
                )
        ...
        for pno, page in enumerate(self.doc):
            ...
            for t in raw_tables:
                ...
                if cols <= 3:
                    ...
                    fitz.Rect(t["bbox"])  # PyMuPDF coords
                    ...
                    if avg_words > 2.5 and digit_ratio < 0.2:
                        self._log(
                            f"Filtered text-heavy table on page {pno+1}: avg_words={avg_words:.1f}, digit_ratio={digit_ratio:.2f}"
                        )
                        continue
                ...
                if b["type"] == 0:
                    ...
                    if is_header_block:
                        ...
                        if text_content.upper().strip().startswith("REQ-"):
                            ...
                            if self.verbose:
                                print(
                                    f"[DEBUG] Demoted REQ- block from Header: '{text_content[:30]}...'"
                                )
                    ...
                    if is_header_block and len(text_content) < 200:
                        ...
                        is_continued = (
                            "(continued)" in text_content.lower()
                            or text_content.strip().lower().endswith("- continued")
                        )
                        ...
                        if is_continued and len(self.spec["sections"]) > 0:
                            ...
                            self._add_debug(
                                pno, fitz.Rect(bbox), (0.5, 0, 0), f"Continued: {text_content[:15]}"
                            )
                        else:
                            ...
                            self.spec["sections"].append(current_section)
                            self._add_debug(
                                pno, fitz.Rect(bbox), (1, 0, 0), f"H{level}: {text_content[:15]}"
                            )
                    else:
                        # Body Paragraph
                        ...
                        if is_short and (has_math or is_labeled) and not is_numbered_start:
                            ...
                            is_requirement = (
                                any(k in text_content.lower() for k in req_keywords)
                                and len(text_content) > 20
                            )
                            ...
                            if is_requirement:
                                ...
                                max_size = (
                                    max(s["size"] for line in b["lines"] for s in line["spans"])
                                    if b["lines"]
                                    else 0
                                )
                                ...
                                if is_bold or max_size >= self.body_size + 1:
                                    ...
                                    self._add_debug(
                                        pno, fitz.Rect(bbox), (0.5, 0, 0.5), "Sub-Header"
                                    )
                                else:
                                    # Regular Body - Mimic with Lorem
                                    word_count = len(text_content.split())
                                    mimic_text = (LOREM * (word_count // 10 + 1))[
                                        : len(text_content)
                                    ]
            ...
            for table_spec in detected_tables:
                ...
                self._add_debug(
                    pno,
                    fitz.Rect(table_spec["bbox"]),
                    (1, 0.5, 0),
                    f"Table {table_spec['table_index']+1}",
                )
        ...
        for pno, page in enumerate(self.doc):
            if pno in self.debug_rects:
                ...
                for rect, color, label in self.debug_rects[pno]:
                    ...
                    # Draw label
                    shape.insert_text(
                        fitz.Point(rect.x0, rect.y0 - 5), label, fontsize=8, color=color
                    )
                shape.commit()
...
def main():
    ...
    parser.add_argument(
        "--debug-visuals", action="store_true", help="Generate visual debug overlays"
    )
