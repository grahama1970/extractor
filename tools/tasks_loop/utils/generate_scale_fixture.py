...
def create_scale_fixture(output_path: Path, page_count: int = 50, config_path: Path = None, seed: int = None):
    # Set Seed
    if seed is not None:
        random.seed(seed)
    ...
    f_name = style.get("font_name", "Helvetica") # PyMuPDF supports limited std fonts unless registered

    for i in range(page_count):
        page = doc.new_page(width=style.get("page_width", 595), height=style.get("page_height", 842))
        ...
        if mode == 0: # Text Heavy
            for b in range(3):
                ...
                page.insert_text((50, y), sub_header, fontsize=f_body+2, fontname=f_name + "-Bold")
                y += 20
                ...
                insert_chaotic_textbox(page, fitz.Rect(50, y, 550, y+100), LOREM, config, fontsize=f_body, fontname=f_name)
