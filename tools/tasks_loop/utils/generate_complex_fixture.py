...
def create_messy_fixture(output_path: Path, config_path: Path = None, image_path: Path = None):
    # Load Config
    config = {}
    ...
    # Insert Sample Figure if provided
    if image_path and image_path.exists():
        img_rect = fitz.Rect(LEFT_COL_START, 100, LEFT_COL_START + 200, 300)
        p1.insert_image(img_rect, filename=str(image_path))
        p1.insert_text(
            (LEFT_COL_START, 310),
            "Figure 1: Technical Flowchart Process",
            fontsize=10,
            fontname="Helvetica-Oblique",
        )
        expected_md_lines.append("![Figure 1: Technical Flowchart Process](sample_figure.png)")
        expected_data.append(
            {
                "id": "FIG-001",
                "type": "Figure",
                "title": "Technical Flowchart Process",
                "text": "Technical Flowchart Process",
            }
        )
        y_after_fig = 330
    else:
    ...
    if layout_mode == "double":
        ...
        p1.insert_textbox(
            fitz.Rect(LEFT_COL_START, y, LEFT_COL_START + COL_WIDTH, y + 60),
            abstract_text,
            fontsize=9,
        )
        y += 70
        ...
        p1.insert_textbox(
            fitz.Rect(LEFT_COL_START, y, LEFT_COL_START + COL_WIDTH, y + 100), bg_text, fontsize=9
        )
        y += 110
        ...
        y2 = y_after_fig
        p1.insert_text(
            (RIGHT_COL_START, y2), "Equations & Math", fontsize=11, fontname="Helvetica-Bold"
        )
        y2 += 20
        # Equation block
        p1.draw_rect(
            fitz.Rect(RIGHT_COL_START, y2, RIGHT_COL_START + COL_WIDTH, y2 + 40),
            color=(0.9, 0.9, 0.9),
            fill=(0.9, 0.9, 0.9),
        )
        p1.insert_text(
            (RIGHT_COL_START + 10, y2 + 25),
            "∮ E · dA = Q / ε₀",
            fontsize=12,
        ...
        p1.insert_text(
            (RIGHT_COL_START, y2), "2. Methodology", fontsize=11, fontname="Helvetica-Bold"
        )
        y2 += 15
        ...
        y2 += 15
        p1.insert_text(
            (RIGHT_COL_START, y2), "[1] Doe, J. 'Scientific PDF Parsing'. 2024.", fontsize=8
        )
    ...
    # Start Table
    y = max(y + 20, 200)
    if y > 600:  # New page if table doesn't fit
        p1 = doc.new_page()
        y = 50

    p1.insert_text(
        (50, y), "Table 1: System Data (Configured)", fontsize=10, fontname="Helvetica-Oblique"
    )
    y += 20
    ...
    p1.insert_text(
        (50, y + 20), "(Table continues on next page...)", fontsize=9, fontname="Helvetica-Oblique"
    )
    ...
    y += 20
    p2.draw_rect(
        fitz.Rect(50, y, 550, y + 20), color=(0.8, 0.8, 0.8), fill=(0.8, 0.8, 0.8)
    )  # Gray header
    ...
    for rid, rtext in reqs:
        ...
        expected_data.append({"id": rid, "type": "Requirement", "text": full_text})
    ...
    if critical_content:
        ...
        has_nesting = (
            "nesting_complexity" in config.get("features", {})
            or config.get("chaos", {}).get("nesting_probability", 0) > 0
        )
        ...
        for item in critical_content:
            ...
            if status:
                p2.insert_text(
                    (50, y), f"[{status}]", fontsize=8, fontname="Helvetica-Bold", color=(0.7, 0, 0)
                )
    ...
    output_obj = {
        "metrics": {
            ...
            "figure_count": len([i for i in expected_data if i["type"] == "Figure"]),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image", type=Path, help="Optional sample figure image")
    args = parser.parse_args()

    create_messy_fixture(args.output, args.config, args.image)
