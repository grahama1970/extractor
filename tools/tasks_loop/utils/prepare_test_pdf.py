...
def add_test_annotations(pdf_path: Path, output_path: Path) -> list[dict]:
    ...
    try:
        ...
        annot0 = page0.add_freetext_annot(
            rect0,
        ...
        annot1 = page0.add_highlight_annot(rect1)
        annotations.append({
            "page": 0,
            "type": "Highlight",
            "bbox": list(rect1),
        ...
        if len(doc) > 2:
            ...
            annot3 = page2.add_text_annot(point, "Review this section for requirements")
            annotations.append({
                "page": 2,
                "type": "Text",
...
def main() -> int:
    ...
    print(f"Applying section numbering fix...")
    # Copy source first
    import shutil
    ...
    print(f"  Fixed: 4.1.5 → 4.1.6")
    
    # Step 2: Add test annotations
    print(f"Adding test annotations...")
