...
from sanity_manifest import get_step_sanity_checks, run_step_preflight
...
STEPS: List[Dict[str, Any]] = [
    {
        ...
        "args": [
            "--pipeline-dir", "data/results/pipeline", 
            "--pdf", "data/input/pipeline/BHT_CV32A65X_test.pdf"
        ],
        "gate": "gate_s01.py",
        "clean": ["data/results/pipeline/01_*"] # Basic cleanup pattern if needed, though mostly handled by steps
    },
    {
        "id": "s02",
    ...
    },
    {
        ...
        "args": [
            "--pipeline-dir", "data/results/pipeline",
            "--pdf-dir", "data/results/pipeline/01_annotation_processor"
        ],
        "gate": "gate_s06.py"
    },
    {
...
from retry_utils import retry_subprocess
...
def run_step(step: Dict[str, Any], dry_run: bool = False, preflight: bool = False, fixture: str = None) -> bool:
...
def update_status(run_id: str, output_root: Path, current_step: str, status: str, error: str = None, fixture: str = None):
...
def main():
    ...
    parser.add_argument("--from-start", action="store_true", help="Run all steps from start up to the target step")
    parser.add_argument("--sanity", action="store_true", help="Run ALL sanity checks first (global)")
    parser.add_argument("--preflight", action="store_true", help="Run step-specific preflight checks (dependency-aware)")
    parser.add_argument("--dry-run", action="store_true", help="Print commands without running")
    parser.add_argument("--fixture", default="BHT_CV32A65X_test", help="Fixture name for contract verification")
    ...
    for i in range(start_idx, end_idx + 1):
        ...
        if not run_step(step, args.dry_run, args.preflight, args.fixture):
            update_status(run_id, output_root, step_id, "FAILED", error="Step Execution Failed", fixture=args.fixture)
            sys.exit(1)
