...
def run_scale_test(iterations: int, pages: int, profile: Path):
    ...
    for i in range(iterations):
        ...
        # 2. Generate
        cmd_gen = ["python3", str(GEN_SCRIPT), str(pdf_path), "--pages", str(pages), "--config", str(profile), "--seed", str(seed)]
        ...
        cmd_verify = ["uv", "run", str(VERIFY_SCRIPT), 
                      "--actual", str(db_path), 
        ...
        results.append(RunResult(seed, pdf_path, True, verify_ok, score, warnings, "" if verify_ok else "Verify Rejected"))
    ...
    for r in results:
        ...
        print(f"{r.seed:<5} | {pipe_str:<6} | {ver_str:<8} | {r.verify_score:<6} | {r.warnings:<9} | {r.error_msg}")
