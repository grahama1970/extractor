...
Single, end‑to‑end checklist consolidating all task files. Check items as you complete them. Screenshot paths follow the convention under `tools/gold_annotator_2/docs/screenshots/<flow>/NN_name.png`.
...
- [ ] JS deps installed: `npm i` in `tools/gold_annotator_2`.
...
- [ ] Worker is reachable at `/pdf.worker.min.js` (served from `public/`).
...
- [ ] Lint and types pass: `npm run lint` and `npm run type-check` (if configured).
  ...
  - Low‑risk guardrails:
    - [ ] Make PDF viewer a client component (`"use client"`).
    - [ ] Serve `pdf.worker.min.js` from `public/` and set `GlobalWorkerOptions.workerSrc = '/pdf.worker.min.js'`.
    - [ ] Lazy import `pdfjs-dist` only in the browser (avoid SSR import).
    - [ ] Any persistence (Dexie) is imported inside client‑only code paths.
...
- [ ] Screenshots: `002_render/01_render_form.png`, `002_render/02_after_render.png`.
  ...
  - Low‑risk guardrails:
    - [ ] Keep only current + adjacent pages rendered; cancel in‑flight renders when navigating.
    - [ ] Virtualize thumbnails; small LRU cache for bitmaps.
...
- [ ] Screenshots: `003_navigate/01_first_page.png`, `003_navigate/02_next_page.png`.
  ...
  - Low‑risk guardrails:
    - [ ] Store last page per PDF (in local storage/IndexedDB) and restore on open.
...
- [ ] Delete with Backspace/Delete (ESC clears selection). Screenshot: `004_boxes/04_box_deleted.png`.
  ...
  - Low‑risk guardrails:
    - [ ] Single source of truth: `toScreen()` and `toNormalized()` utilities for coordinate transforms.
    - [ ] Quick unit test or dev check confirms round‑trip accuracy at 0.5x and 2x.
...
- [ ] For tables, click “Extract”; rows/columns populate if available.
  ...
  - Low‑risk guardrails:
    - [ ] Basic shape checks (keys/types) with a tiny runtime validator (e.g., Zod or manual checks).
...
- [ ] Screenshot: `006_save_boxes/02_reloaded_boxes.png`.
  ...
  - Low‑risk guardrails:
    - [ ] Versioned store key; future migrations add a `schemaVersion` field.
...
- [ ] Click Extract; get text/table JSON or paste fallback. Screenshot: `007_crop_extract/02_extract_result.png`.
  ...
  - Low‑risk guardrails:
    - [ ] Crop uses the same PDF page instance to render an offscreen clipped region at current scale.
