...
@app.command()
def main(
    codebase: Path = typer.Option(
        Path("."),
        exists=True,
        file_okay=False,
        dir_okay=True,
        help="Project directory to pass to the CLI",
    ),
    prompt: str = typer.Option(
        "approaches: mul_shift_add, mul_karatsuba, mul_chunked",
        help="Inline prompt string (alternatively use CLI prompt-file)",
    ),
    codex_bin: str = typer.Option(
        os.environ.get("CODEX_BINARY_PATH", "codex"), help="Path to Codex CLI (or 'codex' on PATH)"
    ),
    ...
    missing = [
        k
        for k in ["ARANGO_HOST", "ARANGO_PORT", "ARANGO_USERNAME", "ARANGO_PASS", "ARANGO_DB"]
    ...
    scoreboard_url = (
        f"{api_base.rstrip('/')}/scoreboard?run_id={run_id}" if api_base else "<unknown>"
    )
