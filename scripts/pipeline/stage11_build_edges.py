...
def _edge_hints_to_edges(hints: dict) -> dict:
    ...
    lemmas = {l["key"]: l for l in hints.get("nodes", {}).get("lemmas", [])}
...
def _flat10_to_edges(flat10: dict) -> dict:
    ...
    for it in items:
        ...
        sections.setdefault(sid, {"key": sid, "doc_id": it.get("doc_id"), "normalized_prop": norm, "polarity": pol})
        # negate-insensitive core
        low = norm.lower()
        core = (
            low.replace("shall not", "shall").replace("must not", "must").replace(" cannot ", " can ").replace(" can't ", " can ")
        )
        core = core.replace(" not ", " ")
        core = " ".join(core.split())
        by_core.setdefault(core, []).append({"section_id": sid, "polarity": pol, "precondition": pre})
        # lemmas
        ...
        for i in range(len(arr)):
            for j in range(i + 1, len(arr)):
                ...
                if ai.get("polarity") and aj.get("polarity") and ai["polarity"] != aj["polarity"]:
                    contradicts.append({"a": ai["section_id"], "b": aj["section_id"], "reason": "opposite_polarity_same_prop"})
...
@app.command()
def main(
    source: Path = typer.Argument(..., help="edge_hints.json from Lean4 or Stage 10 flattened JSON"),
    out_edges: Path = typer.Argument(..., help="Output edges JSON"),
    arango_db: str = typer.Option("", "--arangodb", help="If set, upsert nodes/edges into this DB (requires ARANGODB_URL/USERNAME/PASSWORD)"),
    fallback_lemma_candidates: bool = typer.Option(False, "--fallback-lemma-candidates/--no-fallback-lemma-candidates", help="Use lemma_candidates when used_lemmas is empty"),
):
            ...
            if not edges["edges"]["depends_on"]:
                ...
                for it in items:
                    ...
                    for nm in cand:
                        ...
                        edges["edges"]["depends_on"].append({"from": sid, "to": key, "source": "lemma_candidates"})
    else:
        typer.secho("Unrecognized input; expected edge_hints or Stage 10 flattened JSON", err=True, fg=typer.colors.RED)
        raise typer.Exit(2)
    ...
    if arango_db:
        ...
        for e in edges["edges"]["refines"]:
            ref.insert({"_from": f"sections/{e['refiner']}", "_to": f"sections/{e['refined']}", "reason": e.get("reason")})
