...
def main(out_dir: str) -> int:
    ...
    m["figures.described.count"] = sum(
        1 for f in figures if (f.get("ai_description") or f.get("description"))
    )
    m["headers.suspicious.count"] = sum(
        1 for b in headers if b.get("suspicious_header") or b.get("is_suspicious")
    )
    ...
    m["assessment"] = {
        "figures_described_ratio": (
            (m["figures.described.count"] / m["figures.count"]) if m["figures.count"] else 0.0
        ),
        "reflow_success_ratio": (
            (m["reflow.success.count"] / m["reflow.sections.count"])
            if m["reflow.sections.count"]
            else 0.0
        ),
    }
