...
def stage01(pdf: Path, outdir: Path) -> Path:
    run(
        [
            sys.executable,
            str(PIPE_DIR / "s01_annotation_processor.py"),
            "run",
            str(pdf),
            "-o",
            str(outdir),
...
def stage02(outdir: Path) -> Path:
    ...
    run(
        [
            sys.executable,
            str(PIPE_DIR / "s02_marker_extractor.py"),
            "run",
            str(clean_pdf),
            "-o",
            str(outdir),
...
def stage03(outdir: Path) -> Path:
    ...
    run(
        [
            sys.executable,
            str(PIPE_DIR / "s03_suspicious_headers.py"),
            "run",
            str(s02),
            "--pdf-dir",
...
def stage04(outdir: Path) -> Path:
    ...
    run(
        [
            sys.executable,
            str(PIPE_DIR / "s04_section_builder.py"),
            "run",
            str(s03),
            "--pdf-dir",
            str(outdir / "01_annotation_processor"),
...
def stage05(outdir: Path) -> Path:
    ...
    run(
        [
            sys.executable,
            str(PIPE_DIR / "s05_table_extractor.py"),
            "run",
            str(s04),
            "--pdf-dir",
            str(outdir / "01_annotation_processor"),
...
def stage06(outdir: Path) -> Path:
    ...
    run(
        [
            sys.executable,
            str(PIPE_DIR / "s06_figure_extractor.py"),
            "run",
            str(s02),
            "--sections",
            str(s04),
...
def stage07(outdir: Path) -> Path:
    ...
    cmd = [
        sys.executable,
        str(PIPE_DIR / "s07_assemble_corpus.py"),
        "run",
        "--sections",
        str(s04),
        "--tables",
...
def stage08(outdir: Path) -> Path:
    ...
    run(
        [
            sys.executable,
            str(PIPE_DIR / "s08_extract_requirements.py"),
            "run",
            "--corpus",
            str(s07),
            "-o",
    ...
    run(
        [
            sys.executable,
            str(PIPE_DIR / "s08_lean4_theorem_prover.py"),
            "run",
            "--db",
            str(db_path),
            "-o",
            str(outdir / "08_lean4_theorem_prover"),
        ]
    )
...
def stage09(outdir: Path) -> Path:
    ...
    run(
        [
            sys.executable,
            str(PIPE_DIR / "s09_llm_enrichment.py"),
            "run",
            "--corpus",
            str(s07),
            "-o",
...
def stage10(outdir: Path) -> Path:
    ...
    run(
        [
            sys.executable,
            str(PIPE_DIR / "s10_markdown_exporter.py"),
            "run",
            "--corpus",
            str(inputs),
            "-o",
