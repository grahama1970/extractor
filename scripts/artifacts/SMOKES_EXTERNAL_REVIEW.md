# Extractor — Targeted Review (Smokes Rinse, Preview Console 500s) — 2025-09-27

Status
- After three attempts, `scripts/smokes/console_errors.mjs` still fails on preview due to console 500s.
- UX health gate passes (pointerDrawOk=true, toolbarClear=true), but console_errors is stricter.

Artifacts
- console_errors log: `/home/graham/workspace/experiments/extractor/scripts/artifacts/console_errors_2025-09-27T15-55-40-104Z.log`
- console_errors screenshot: `/home/graham/workspace/experiments/extractor/scripts/artifacts/console_errors_2025-09-27T15-55-40-104Z.png`
- last ux:check log: `/home/graham/workspace/experiments/extractor/scripts/artifacts/ux_check_cdp_2025-09-21T14-43-39-289Z.log`
- last ux:check screenshot: `/home/graham/workspace/experiments/extractor/scripts/artifacts/ux_check_cdp_2025-09-21T14-43-39-289Z.png`

What we changed in this PR (already landed)
- Gate `/api/build` fetch behind `import.meta.env.DEV` to avoid preview 500s.
- In preview (non‑dev), skip `/api/list` and set a stub PdfDoc directly.
- Teach `loadPdf('__stub__')` to avoid all network fetches (no pdf.js request) and draw placeholders.
- Skip `ensureWorker()` when rendering a stub (avoid dynamic import noise).

Remaining failure
- console still shows generic 500 messages without URLs under preview; source likely a lingering `/api/*` fetch or a resource URL resolved by some component.

Proposed minimal next step
- Add an `isPreview()` helper and use it to prevent any `/api/*` fetches at mount across the app.
- Instrument `scripts/smokes/console_errors.mjs` to include requestfailed URLs in the log to pinpoint the exact source.

Acceptance
- `node scripts/smokes/console_errors.mjs` exits 0 under preview (no console.error entries).
- `make rinse` remains green; CDP attach variant also green.
...
function BuildChip() {
  ...
  if (!(import.meta as any)?.env?.DEV || !data) return null;
...
// Lightweight PDF.js integration helpers for the prototypes.
// Uses pdfjs-dist v4 and serves the worker from public/pdf.worker.min.js

// We intentionally keep types minimal to avoid tight coupling to pdfjs-dist's internal types.
export type PdfDoc = {
  numPages: number;
...
function ensureWorker() {
  ...
  try {
    // Lazy import to keep tree-shaking friendly in Vite
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const pdfjs: any = require("pdfjs-dist");
    if (pdfjs?.GlobalWorkerOptions) {
      // Serve worker from public/
      pdfjs.GlobalWorkerOptions.workerSrc = "/pdf.worker.min.js";
      workerConfigured = true;
    }
  } catch {
    // ESM fallback
    // @ts-ignore - dynamic import may not be typed here
    import("pdfjs-dist").then((pdfjs: any) => {
      if (pdfjs?.GlobalWorkerOptions) {
        pdfjs.GlobalWorkerOptions.workerSrc = "/pdf.worker.min.js";
        workerConfigured = true;
      }
...
// Attempt to load a PDF document. If the URL fails, returns a small stub-like object
// so the UI still renders with graceful placeholders.
export async function loadPdf(url: string): Promise<PdfDoc> {
  // Special-case stub to avoid network fetch and console errors in preview
  const makeStub = (): PdfDoc & { __stub?: boolean } => ({
    numPages: 2,
    async getPage() {
      return {
        ...
        render: ({ canvasContext, viewport }: any) => ({
          promise: new Promise<void>((resolve) => {
            ...
            ctx.fillText("Preview Placeholder (no PDF)", 24, 36);
            ctx.restore();
            resolve();
          }),
        }),
  ...
  try {
    ...
    const loadingTask = mod.getDocument({ url, verbosity: 0 });
    const doc = (await loadingTask.promise) as PdfDoc;
    return doc;
  } catch (err) {
    console.warn("[pdf] Failed to load", url, err);
    return makeStub();
  }
}

export async function renderPageCanvas(doc: PdfDoc, pageNum: number, canvas: HTMLCanvasElement, zoom = 1, signal?: AbortSignal) {
  if (!(doc as any).__stub) ensureWorker();
  const page = await doc.getPage(pageNum);
  const dpr = typeof window !== 'undefined' ? Math.max(1, Math.min(window.devicePixelRatio || 1, 3)) : 1;
  const vp = page.getViewport ? page.getViewport({ scale: zoom * dpr }) : { width: 800 * zoom * dpr, height: 1000 * zoom * dpr };
  const cssWidth = Math.max(1, Math.floor(vp.width / dpr));
  const cssHeight = Math.max(1, Math.floor(vp.height / dpr));
  const ctx = canvas.getContext("2d");
  if (!ctx) return;
  // Set backing store to DPR-scaled size for crispness
  canvas.width = Math.max(1, Math.floor(vp.width));
  canvas.height = Math.max(1, Math.floor(vp.height));
  // Set CSS size in layout pixels
  canvas.style.width = `${cssWidth}px`;
  canvas.style.height = `${cssHeight}px`;
  if ((ctx as any).setTransform) (ctx as any).setTransform(1, 0, 0, 1, 0, 0);
  if (page.render) {
    ...
    try {
      ...
      // Ignore expected cancelation
      if (!(e && (e.name === 'RenderingCancelledException' || e.message?.includes('Rendering cancelled')))) throw e;
    }
  } else {
    // Fallback: draw simple sheet
    ctx.fillStyle = "#fff";
...
export async function renderPageThumbnail(doc: PdfDoc, pageNum: number, targetWidth = 120): Promise<string | undefined> {
  try {
    // If this is a stub document, signal caller to retry later
    if ((doc as any).__stub) return undefined;
    const page = await doc.getPage(pageNum);
    const base = page.getViewport ? page.getViewport({ scale: 1 }) : { width: 612, height: 792 };
    const scale = Math.max(0.1, targetWidth / (base as any).width);
    const canvas = document.createElement("canvas");
    await renderPageCanvas(doc, pageNum, canvas, scale);
    // Heuristic: reject near-blank thumbnails (<1% non-white pixels)
    try {
      const ctx = canvas.getContext("2d");
      if (ctx) {
        ...
        for (let y = 0; y < h; y += step) {
          for (let x = 0; x < w; x += step) {
            ...
            // Count as non-white if any channel significantly below 255
            if (r < 245 || g < 245 || b < 245) nonWhite++;
            total++;
          }
        }
        const ratio = total ? nonWhite / total : 0;
        if (ratio < 0.01) return undefined;
      }
    } catch {}
    return canvas.toDataURL("image/png");
  } catch (err) {
    // SVG data URL placeholder thumbnail
    const svg = `<svg xmlns='http://www.w3.org/2000/svg' width='${targetWidth}' height='${Math.round(
      (targetWidth * 4) / 3,
    )}' viewBox='0 0 ${targetWidth} ${Math.round((targetWidth * 4) / 3)}'>
      <rect x='0' y='0' width='100%' height='100%' fill='#ffffff'/>
      <rect x='0.5' y='0.5' width='${targetWidth - 1}' height='${Math.round((targetWidth * 4) / 3) - 1}' fill='none' stroke='#e5e7eb'/>
      <text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' fill='#64748b' font-size='12' font-family='system-ui'>No PDF</text>
    </svg>`;
    return `data:image/svg+xml;utf8,${encodeURIComponent(svg)}`;
  }
}

// Extract plain text from a page using pdf.js textContent.
...
// Build a naive text index across pages. Caller can abort via AbortSignal.
export async function buildTextIndex(doc: PdfDoc, onProgress?: (done: number, total: number) => void, signal?: AbortSignal): Promise<Record<number, string>> {
  const total = doc?.numPages || 0;
  const out: Record<number, string> = {};
  for (let i = 1; i <= total; i++) {
    if (signal?.aborted) break;
    out[i] = await getPageText(doc, i);
    if (onProgress) onProgress(i, total);
  }
  return out;
}

// Very naive query → highlight boxes extractor using pdf.js textContent items.
// Returns normalized boxes (0..1) approximating the line bounding boxes that include the query.
export async function getQueryBoxes(doc: PdfDoc, pageNum: number, query: string): Promise<{x:number;y:number;w:number;h:number}[]> {
  ensureWorker();
  ...
  try {
    ...
    const page: any = await doc.getPage(pageNum);
    const vp = page.getViewport ? page.getViewport({ scale: 1 }) : { width: 612, height: 792 };
    if (!page?.getTextContent) return out;
    const tc: any = await page.getTextContent({ normalizeWhitespace: true });
    const items = Array.isArray(tc.items) ? tc.items : [];
    for (const it of items) {
      const s: string = String(it.str || '');
      if (!s) continue;
      if (s.toLowerCase().includes(q)) {
        // Approximate bounds from transform and width
        const tr = Array.isArray(it.transform) ? it.transform : [1,0,0,1,0,0];
        const x = Number(tr[4]) || 0; // e
        const y = Number(tr[5]) || 0; // f (baseline)
        const wpt = Number(it.width) || (s.length * 6);
        const hpt = Math.abs(Number(tr[3]) || Number(it.height) || 10);
        // pdf.js y=baseline; shift up a bit to cover text height
        const x0 = x;
        const y0 = Math.max(0, y - hpt*0.8);
        const nx = Math.max(0, Math.min(1, x0 / (vp as any).width));
        const ny = Math.max(0, Math.min(1, y0 / (vp as any).height));
        const nw = Math.max(0.002, Math.min(1, wpt / (vp as any).width));
        const nh = Math.max(0.005, Math.min(1, hpt / (vp as any).height));
        out.push({ x: nx, y: ny, w: nw, h: nh });
      }
    }
  } catch {
    // ignore errors; return whatever collected
  }
  return out;
}
