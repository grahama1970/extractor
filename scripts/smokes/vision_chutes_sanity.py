...
def _run(
    model: Optional[str], image_url: str, api_base: str, api_key: str, timeout: float = 20.0
) -> tuple[bool, str]:
    ...
    try:
            ...
            "input": [
                {
                    ...
                    "content": [
                        {
                            "type": "input_text",
                            "text": 'Return only {"ok": true, "desc": string} as JSON.',
                        },
        ...
        data = _http_request(
            "POST", f"{api_base.rstrip('/')}/responses", api_key, payload_resp, timeout
        )
...
def main(
    ...
    out.write_text(
        json.dumps({"ok": ok, "model": mdl, "image": image_url, "content": content}, indent=2),
        encoding="utf-8",
    )
    typer.echo(str(out))
