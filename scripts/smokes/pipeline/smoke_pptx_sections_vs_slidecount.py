...
def main(
    ...
    if slide_count == 0:
        typer.echo("PPTX unified_document.metadata.format_metadata.slide_count missing or zero.", err=True)
        raise typer.Exit(code=1)
    if len(sections) != slide_count:
        typer.echo(f"Section count mismatch: sections={len(sections)} slide_count={slide_count}", err=True)
        raise typer.Exit(code=1)
