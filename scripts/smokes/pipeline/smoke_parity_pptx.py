...
def main(
    ...
    pptx_path: Path = typer.Option(Path("data/results/pipeline/01_annotation_processor/bht_formats/BHT_CV32A65X_marked_clean.pptx"), exists=True),
    ...
    artifacts = run_structured_pipeline(PPTXProvider, pptx_path, results_dir, stage_prefix=meta.stage_prefix, skip_export10=True, skip_embeddings10=True, fast_embeddings10=True)
    ...
    summary_path.write_text(json.dumps({
        ...
        "inputs": {"pdf_stage07": str(pdf_stage07), "pptx_path": str(pptx_path), "pptx_flattened": str(artifacts["stage10_flattened"])},
    }, indent=2))
    ...
    typer.echo(f"PPTX parity presence checks passed (sections-only + section-context). sections={len(pptx_sections)}")
