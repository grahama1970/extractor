...
def run(model: Optional[str] = typer.Option(None, "--model", help="Override text model id")):
    require_scillm_env()
    mdl = (model or os.getenv('LITELLM_DEFAULT_MODEL') or os.getenv('LITELLM_SMALL_TEXT_MODEL') or '').strip()
    if not mdl:
        print(json.dumps({"ok": False, "error": "No text model configured"}, indent=2)); sys.exit(2)
    try:
        ...
        print(json.dumps({"ok": False, "error": str(e), "model": mdl}, indent=2)); sys.exit(4)
